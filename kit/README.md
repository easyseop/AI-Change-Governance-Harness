...
> **상태:** Draft (MVP-0·1·1.5·2 + MVP-3 Java J0~J3 + Java L3 partial 반영). **개발 작업과 분리** — 이 폴더는 `sync-from-dev.sh` 로 생성되는 스냅샷이다.
...
**언어 라우팅/J0 스모크 + J1 Java 인벤토리**(spine hook 아님 — coverage 정직성·배포환경 실증):
`language-router` · `check-tree-sitter-languages` · `extract-java-inventory`.

→ 판정 7 + 추출 9 + 도구 2 + 언어 J0 2 + 언어 J1 1 = **21종 전량**. `manifest.yaml` `gates:` 에 명시.
Tree-sitter 기반 스모크를 배포지에서 재현하려면 `python3 -m pip install -r requirements.txt` 를 먼저 실행한다.

Java L3 간접영향 분석은 배선되어 실제 판정에 참여하지만 정책 상태는 **`partial`** 이다. 보수적
전방폐쇄에서 sink 가 닿는 외부 호출이 하나라도 있으면 지연 dispatch 가 승인요구로 승격될 수
있어(O-14) 소음이 발생한다. 결과의 `inferred: true` 는 실제 호출을 관측했다는 뜻이 아니라
**보수 추정 홉이 경로에 포함됨**을 뜻한다. `fail_closed`의 `dead_ends=`에는 해당 레코드와
무관한 이름이 함께 열거될 수 있다(O-22). 이 한계를 포함해 Java L3를 안전 지원으로 과장하지 않는다.
...
4. **Java 콜그래프는 partial이며 JS/TS 심층 판정·cross-commit은 미포함.** 경로층은 언어무관
   동작하고, Java는 인벤토리·함수 민감도·신규 능력과 보수적 L3 간접영향까지 판정한다. Java L3의
   O-14/O-22 및 `inferred` 의미는 위 제한을 따른다.
...
- **MVP-3 J3** Java 신규 민감 능력 판정
- **MVP-3 Java L3 (partial)** Java sink·콜그래프 기반 간접영향 판정과 보수적 coverage 노출
