...
> **상태:** Draft (MVP-0·1·1.5·2 + MVP-3 J0 language routing seam + J1 Java inventory 반영). **개발 작업과 분리** — 이 폴더는 `sync-from-dev.sh` 로 생성되는 스냅샷이다.
...
4. **Java/JS/TS 심층 판정·cross-commit** 은 미포함. 경로층은 언어무관 동작하고,
   MVP-2 간접영향 추적은 단일 diff 내 Python 정적 호출만 분석한다.
   MVP-3 J1 는 Java 클래스/메서드/생성자 인벤토리까지 포함하며, Java `@Gov`/Spring 어노테이션·능력·콜그래프 판정은 아직 stub coverage 로 노출한다.
