...
> **상태:** Draft (MVP-0·1·1.5·2 + MVP-3 J0 language routing seam 반영). **개발 작업과 분리** — 이 폴더는 `sync-from-dev.sh` 로 생성되는 스냅샷이다.
...
**언어 라우팅/J0 스모크**(spine hook 아님 — coverage 정직성·배포환경 실증):
`language-router` · `check-tree-sitter-languages`.

→ 판정 7 + 추출 7 + 도구 2 + 언어 J0 2 = **18종 전량**. `manifest.yaml` `gates:` 에 명시.
...
4. **비-Python 함수수준 영향추적·cross-commit** 은 미포함. 경로층은 언어무관 동작하고,
   MVP-2 간접영향 추적은 단일 diff 내 Python 정적 호출만 분석한다.
   MVP-3 J0 는 확장자 라우팅과 tree-sitter 의존 스모크만 포함하며, Java/JS/TS 정밀 분석은 아직 stub coverage 로 노출한다.
