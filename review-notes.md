...
## TASK-034 보정(`9ecad20`) 재리뷰 — 통과 (2026-07-19 · D-068)

**대상**: 보정 커밋 `9ecad20` 만(멱등 — `22fe433`·`a4037ce` 재처리 금지).

### 한 줄씩 확인
`intent_not_declared_evidence()` 삭제(카드 대체 경로 제거) · `build_evidence()` 가 `IntentNotDeclaredError` 를 `try` 로 흡수해 **빈 선언으로 계속**(합성 `["**"]` 주입 아님) · `not_declared_intent_result()` 가 파생결과(`out_of_scope`/`missing_expected`/`forbidden_touched`) 비움 = 빈 선언 과탐 차단 · `verdict_and_exit` 는 frozen/forbidden 을 **먼저** 판정(`blocked > not_declared`) · `combine_verdicts` 가 `function_gov: blocked` 독립 반영 · `in_allowed_paths: None` · `evidence_checked_records()` 는 `check-change-intent` **만** 제외. 예외 클래스가 `FileNotFoundError` 하위이며 **파일 부재에서만** raise → 잘못된 YAML·디렉토리는 기존 fail-closed 유지(우회 표면 확대 없음).

### 적대적 검증 (픽스처 밖 fresh · pre-fix 대비 delta)
frozen 경로+미선언 **2 → 1** · `@gov(frozen)` 함수+미선언 **2 → 1** · 무해+미선언 **2**(AC#1) · 빈 선언 **exit 2·`status: fail`·`in_allowed_paths: false`**(미선언과 구분). 킷 레이아웃 게이트 직접 실행 + **E2E `kit/run.sh` = 🔴 BLOCKED(exit 1)·전층 실행**.

### 정직성 (R-2)
미선언 카드에 실제 `base_commit`·`policy_sha`·`files_changed: 1`·`frozen_touched`(사유 포함)·`checked: [check-sensitive-zones, check-function-gov-level]`. 승인자가 정산 변경 사실을 카드에서 직접 본다.

### 음성검증
RIG-1(판정층 R-1 재유입) → frozen-zone 케이스 **단독 FAIL(97/98)** · RIG-2(`in_allowed_paths` 위조 재유입) → 미선언 3케이스 **전부 FAIL(95/98)**. 가드 load-bearing.

### 재현
98/98 · mutation 165 · 진입점 13/13 · selftest PASS · 게이트 16종 md5 동일. AC **6/6**.

### 보수적 개발
scope-creep·over-reach 없음. D-067 의 over-reach(판정 완화)가 제거되고 **차단 복원(안전 방향)** 으로 전환. blast radius = 의도 일치.
...
## TASK-029 R-5 보정(`117b4a3`) 재리뷰 — **통과 · 머지** (2026-07-20)

### 무엇을 능동적으로 깨보려 했나
- **RIG (가드 하중 시험)**: `kit/run.sh` preflight 에서 `"$LANGUAGE_ROUTING"` 항목만 삭제 → 진입점 **14/15 단독 FAIL**. 실패 출력에 `verdict: blocked` + `reasons: language routing policy missing` 이 찍혀 **R-4 계열 회귀가 그대로 재현**. A-0028 시점엔 이 조작이 **무음**이었다 ⇒ 죽은 가드가 실제로 살아났다.
- **음성검증**: 신규 케이스 기대 rc 를 `2 → 3` 으로 변조 → 단독 FAIL. 항상-PASS 아님.
- **픽스처 밖 fresh 입력**: 내가 직접 만든 repo(정책 4종만 둔 override · 무해 Python 1함수 추가)로 실행 → `✗ 분석 실패 · exit 2 · 카드 미생성`. README 안내대로 정책 1파일 복사 후 → `exit 0 · verdict: pass`. **문서가 실제 탈출구로 작동**함까지 확인(문서 리뷰를 문장 검토로 끝내지 않음).
- **sync 실작동**: `sync-from-dev.sh` 재실행 → 게이트 18종 누락 0 · `git status` clean(멱등) · 킷 전용 진입점 시험이 스냅샷/복원 경로(`sync-from-dev.sh:48–54`)로 **생존**. 개수만 세지 않고 신규 케이스 생존을 직접 확인.
- **문서-코드 드리프트**: `run.sh:48` · `manifest.yaml:61–66` · README 세 곳의 필수 정책 목록이 **동일 5종**으로 일치.

### 왜 안전한가 (무발견 ≠ 통과)
신규 케이스는 exit 코드·실패 메시지·카드 미생성을 **3중으로** 단언한다. 이게 중요한 이유: `ACGH_GATE_TIMEOUT_SECONDS=1` 환경에서 타임아웃도 exit 2 를 낼 수 있어 **rc 단독 단언이면 위양성**이 난다. 메시지 단언이 그 구멍을 막는다 — 음성검증에서 rc 만 변조했을 때 메시지 단언이 계속 판정을 붙잡는 것을 확인했다.

### 내가 짠다면
A-0028 의 "정책 경로를 한 배열로 모아 preflight·전달을 동일 루프로" 안이 여전히 근본책이지만, 이번 보정의 책임 범위(시험 추가)를 넘는 리팩터라 강요하지 않았다(§2B 균형). 케이스 자체는 내가 짜도 거의 같은 모양. 미세 흠: FAIL 메시지가 `expected=2` 하드코딩이라 기대값 변조 시 `exit=2 expected=2` 자기모순 문구 — 표시 전용, 비차단.

### 하류 영향
- R-1~R-5 전량 해소 ⇒ **TASK-030(J1 Java 추출기) 착수 전제 충족**. J1 이 `language-routing.yaml` 에 Java 항목을 넣을 때 이번 preflight 회귀가 "정책 파일 소실" 배포 사고를 계속 막는다.
- 킷 도입처는 업그레이드 시 **1회 마이그레이션(정책 1파일 복사)** 필요 — 누락해도 조용한 오판정이 아니라 즉시 정직한 분석 실패라 안전하며, 이제 README 에 명시됐다.

### 차기 AC 가드 (비차단 관찰 아님)
**G-1**: preflight 5항목 중 시험 보유는 2개뿐. `sensitive-zones`·`sensitive-capabilities`·`approval-routing` 은 **R-5 와 같은 죽은 가드** — 실증으로 `"$ZONES"` 제거 시 `exit 1 / verdict: blocked` 오판정 부활을 확인했다. **선재 갭**이라 보정요청 대신 **차기 태스크 AC 로 명문화**(§2B: 보정요청 *또는* 차기 AC 가드): preflight 를 표 주도 회귀로 전환해 5항목 각각 3중 단언 + 항목별 RIG 단독 FAIL 실증.

---

## TASK-030 — Java 함수/메서드 인벤토리 추출기 (J1) · 리뷰 → 보정요청 R-1 (D-073 / A-0030)

### 한 줄
게이트 자체는 잘 짜였다. **깨진 건 게이트가 아니라 게이트가 의존을 잃었을 때의 태도**다.

### 능동적으로 깨보려 한 것 (통과 = 안 깨진 결과)
1. **오버로드 적대 입력**(픽스처 밖 fresh): 신규 오버로드를 기존 것 *앞에* 삽입 + `@PreAuthorize`
   인자만 변경 + 나머지 오버로드 무변경. 이름 단독/순서 단독 키였다면 세 건 다 오판할 입력인데
   전부 정확히 나왔다. **매칭키에 시그니처를 넣은 설계가 옳다.**
2. **CLAUDE.md 고정 적대 세트**(데코레이터·동명 오버로드·조건부/중첩 정의)의 Java 대응을 직접 작성:
   제네릭·varargs·익명 내부클래스·enum/record/interface·메서드 내 로컬 클래스. 파싱은 전부 살아남았고,
   여기서 O-1(익명/로컬 클래스 이름 손실)이 나왔다.
3. **음성검증 2건** — 매칭키에서 시그니처를 빼면 무변경 오버로드가 허위 modified,
   `_signature_dump` 에서 어노테이션을 빼면 인가 어노테이션 인자 변경이 **통째로 사라진다**.
   둘 다 단독 FAIL(106/107) ⇒ 신규 가드가 죽어있지 않다.

### 결함을 찾은 경로 (기록해 둘 가치가 있음)
킷 `selftest.sh --quick` 이 FAIL 했다. 첫인상은 "내 환경 노이즈" 였고 실제로 main 도
`tree-sitter-smoke` 하나는 FAIL 하고 있었다 — **여기서 멈췄으면 놓쳤다.** 귀책을 가르려고
브랜치/main 진입점 스위트를 3회씩 돌리자 **브랜치 3/3 FAIL vs main 3/3 PASS** 로 갈렸고,
FAIL 출력 tail 을 잘라내지 말고 카드 전문을 찍자 `reasons` 에 dlopen 아키텍처 불일치가 나왔다.
그다음은 환경 재현이 아니라 **`import tree_sitter` 를 강제로 실패시키는 shim** 으로 옮겨
환경 무관하게 재현했다 — "내 맥북 문제" 를 "배포 계약 문제" 로 승격시킨 지점.

교훈 두 가지: ① **선재 FAIL 이 있어도 귀책 분해를 포기하지 말 것**(main 대비 반복 실행이 가른다),
② **FAIL 요약이 아니라 카드 전문을 볼 것**(tail -16 안에 원인이 없었다).

### 왜 이건 비차단으로 못 미루나 (§2B 필수질문)
"의존이 없으면 못 분석한다" 는 정직한 상태다. 문제는 그 정직한 상태를 **`verdict: blocked` +
`governance violation detected`** 로 말한다는 것. 민감영역은 하나도 안 건드렸는데 위반이라고 한다 =
**허위 사유**. 여기에 반대방향 구멍이 겹친다 — `.java` 하나가 같은 diff 의 **Python 함수 매핑 전체**를
`files: []` 로 날린다. 과차단과 과소탐지를 동시에 내므로 비차단 불가.

AC#4 는 이미 "파싱 실패 → 파일 단위 격리 · 형제 보존 · exit 0" 을 요구한다. 의존 부재는 문법오류보다
**흔한** 실패인데 그 규약 밖에 있었다. 보정은 새 설계가 아니라 **기존 규약의 적용 범위를 넓히는 것**이라
blast radius 가 작다(게이트 2파일 + 픽스처).

### 내가 짠다면
거의 같게 짰을 것이다. 한 군데만 다르다: `extract_java_inventory` 를 **"파서를 얻지 못하면 그것도
parse_error"** 로 처음부터 정의했을 것 같다. 지금 구조는 `has_error` 만 parse_error 로 보고
"파서 자체가 없음" 은 예외로 던져 상위에 맡기는데, 상위 두 곳(map-diff·classify)이 서로 다른
캐치 튜플을 갖고 있어서 **한쪽은 fallback, 한쪽은 `files: []`** 로 갈렸다. 실패 표현을 게이트 안에서
하나로 모으면 이 비대칭이 생기지 않는다.

### 하류 영향
- Java IR·오버로드 키·어노테이션 매핑은 **TASK-031(J2) 착수 전제로 충분**.
- 단 J2 는 어노테이션을 **이름으로** 민감도에 귀속시킨다 → O-1(익명 내부클래스가 `Outer.run` 이 되는 것)이
  그때 **잘못된 함수에 인가 등급을 붙이는 실해**로 바뀐다. 비차단으로 흘리지 말고 **TASK-031 AC 의
  고정 적대 픽스처로 명시**하는 이유다.
- routing 의 java 는 여전히 `status: stub` 이라 카드는 실제보다 **적게** 주장한다(안전 방향이지만 부정확).
  파서/문법 버전 기록(O-A)과 함께 J2 에서 정리해야 카드가 현실과 맞는다.

---

# TASK-030 R-1 보정(`1b11550`) 재리뷰 — 2026-07-20 (Claude)

## 무엇을 깨보려 했나
검증환경에 `tree_sitter` 가 아예 없다(`ModuleNotFoundError`). R-1 당시의 dlopen 아키텍처 불일치와
같은 ImportError 계열이라, **shim 없이 실제 실패경로**를 그대로 밟을 수 있었다. 픽스처를 신뢰하지 않고
독립 repo 를 새로 만들어(`app/auth/Guard.java` 인가 어노테이션 약화 + 같은 diff 에 `svc/calc.py`)
main 과 브랜치를 나란히 돌렸다.

## 결과
카드가 **바이트 동일**하게 나왔고 둘 다 `approval_required`. 민감 사유는 경로 기반이라
파서 가용성과 완전히 분리돼 있다 — R-1 증상 C(허위 사유)의 구조적 원인이 사라졌다는 뜻이다.
혼합 diff 에서 `.java` 만 `parse_error` 로 격리되고 `settle` 매핑이 살아남았다(증상 D 해소).

## 가장 배운 점 — 중복 가드는 개별 rig 로 죽지 않는다
`#3`(게이트 내부) 하나만 있어도 의존부재는 막힌다. 그래서 `#1`(map-diff)을 지워도 스위트가 조용했고,
하마터면 "#1·#2 는 죽은 가드" 로 오판할 뻔했다. 실제로는 **실패모드가 다르다**:
`#3` 은 *의존이 없을 때*, `#1·#2` 는 *co-located `extract-java-inventory.py` 자체가 없을 때*(킷 부분배포)
를 덮는다. 후자를 재현하니 `#1/#2` 없으면 전역 `ERROR:` 로 파일이 전량 소실됐다.
⇒ **가드가 안 죽는다고 불필요한 게 아니라, rig 가 그 가드의 실패모드를 안 밟은 것일 수 있다.**
다만 그 경로에 **픽스처가 0건**이라 회귀로 고정돼 있지 않다 → O-4 로 TASK-031 AC 에 넘긴다.

## 내가 짠다면
D-073 에서 내가 "실패 표현을 게이트 안에서 하나로 모으라" 고 썼는데 보정이 정확히 그렇게 했다.
상위 캐치를 남긴 것도 과잉이 아니다(위 문단). `OSError` 를 넓게 잡는 게 걸렸지만, 강등 결과가
`file_fallback` = 파일 전체를 변경으로 간주하는 **보수적** 방향이라 fail-open 이 아니다.

## 하류 영향 — J2 에서 터질 것
`language_coverage()` 는 `not_checked` 를 **정책 status 유래**로만 만든다. 실제 `parse_error` 는
카드에 도달하지 않는다. J1 은 java 가 `stub` 이라 카드가 "미구현" 을 고지하므로 우연히 정직하지만,
**J2 가 java 를 `supported` 로 올리는 순간 카드는 하지도 않은 심층분석을 보증**하게 된다.
tree-sitter 가 깨진 배포처에서 그 카드는 승인자를 정확히 반대로 오도한다.
R-1 item 6(권장)이었던 "가용여부 진단" 을 **O-5 로 승격해 J2 AC 에 못박는 이유**다.


---

## TASK-031 (D-075) — J2 프레임워크 어노테이션 정책 · Q-0005 답변 (2026-07-20)

**브랜치 스캔 결과**: `git fetch origin --prune` 후 `git branch -r --no-merged origin/main` = **`codex/2026-07-20-task031-java-gov-annotations` 1건뿐**.
- 분류 = **(가) 리뷰 차례**로 잡혔으나, 실제 델타는 `collab/questions/Q-0005.md` **20줄 추가 1커밋**(`4dc0b7c`) = **구현 코드 0줄**.
  ⇒ 리뷰 통과/보정요청이 아니라 **정책 답변**이 정확한 처리. handoff-log 최신 줄은 TASK-030 **리뷰 통과·머지 완료**(D-074)라 (나) 보정대기 브랜치 없음.
- 작업트리에 TASK-031 구현 WIP(**미커밋**: `extract-java-gov-annotations.py`·`tests/fixtures/java-gov/`·`tests/parity/*`·`local-anonymous.java`)가 있으나 **제출물이 아니므로 리뷰 대상 아님** — 무접촉 유지.

**한 것**: `policies/framework-annotations.yaml` 작성(Claude 소유 영역 · 18항목). 게이트 코드는 여전히 미작성(CLAUDE.md §5).

**실측 검증(가정 아님)**: 정책 추가 상태 `tests/run-tests.sh` **110/111** ↔ 파일 제거 baseline **110/111** — 유일 FAIL `tree-sitter-smoke` 동일.
이 환경은 `tree_sitter` 미설치(D-074 에서 확정된 선재 환경 FAIL)이며, **두 실행이 완전 동일** ⇒ 신규 정책이 현 시점 판정에 **inert** 함을 실증.
`policy_sha` 는 인자 열거식(`generate-change-evidence.py:687`)이라 디렉터리 스캔이 아님 → 정책 1개 추가로 카드 digest 가 흔들리지 않음도 확인.

**설계 판단 3건(하류 영향 추적 결과)**:
1. **마지막 점-세그먼트 매칭 요구** — Java 는 FQN 인라인 어노테이션을 허용한다. Codex 제안대로 원문 토큰과 simple name 을 비교하면 `@org.springframework...PreAuthorize` 를 **놓친다**. 놓침 = MVP-3 공통 "과탐 반올림·과소탐 금지" 정면 위반이라 비차단으로 흘릴 수 없다.
2. **인자 조건 `when` 이 스키마에 없으면 `@Query` 가 무너진다** — AC#2 는 `nativeQuery=true` 만 protected 인데 제안 계약엔 인자를 볼 자리가 없다. 결과는 "전부 protected(신호 소멸)" 또는 "전부 무시(원시 SQL 놓침)" 양자택일. 미해소 값은 **추정 금지 + 매칭 취급**으로 고정.
3. **`status: supported` 단순 플립 불승인** — `language-router.py:117` 이 스칼라 `status` 를 `deep_analysis: available` 로 직역한다. J2 시점 java 의 진실은 inventory ✅ / gov_level ✅ / **capabilities ❌** / callgraph ❌ 이므로 플립하면 카드가 **하지 않은 능력분석을 보증**한다(`Runtime.exec` 신규 도입이 무신호 통과인데 카드는 "available"). D-074 O-5 가 예고한 실패의 구체화 ⇒ 층별 `layers:` + **실측 가용성 노출**을 플립의 하드 전제로 승격.

**§2B 필수질문("거버넌스 목적에 직접 구멍을 내나")**: 위 1·2·3 모두 **예**(1·2 = 과소탐, 3 = 허위 카드 + 조용한 통과) ⇒ 비차단 관찰로 미루지 않고 **TASK-031 AC#8·#9 로 명시 고정**.

**재발 패턴 경고**: `kit/run.sh` 정책 배선 누락은 TASK-029 **R-4** 로 이미 한 번 킷 사용자 전원 상시차단을 만들었다. 신규 정책이 생길 때마다 같은 자리가 비므로 AC#8(f) 에 **배선 + preflight** 를 명시했다 — 3회차 금지.
...
## 2026-07-22 — TASK-040 A-0050 보정 재제출 재리뷰 (`1164ce0`·`64e7777`) → 🔴 보정요청 (A-0051 · D-095)

- **범위·멱등성**: `4573b51`·`ea0191c` 는 A-0050 처리 완료 → **재리뷰 없음**. 대상은 `ea0191c..64e7777` 델타뿐. 미머지 codex 브랜치는 이 1개뿐.
- **AC#6 ✅ 종결**: `caller ∈ sink_reachable` → `if not dispatch_targets and sink_dead_ends:` 교체 + `sink_reachable_functions` 완전 제거. **RIG-1 → 175/192, 신규 6건 전부 FAIL**. **픽스처 밖 fresh F1~F5·F7·F8 7형태가 `main` 과 동일한 `approval_required` 로 복구**. N4 개명·정정도 지시대로.
- **AC#7 ✅ 종결**: **RIG-2(교집합→truthiness) → 191/192, `…intersection-pass` 단독 FAIL** — A-0050 의 "184/184 무변화" 공백이 정확히 닫혔다. 픽스처가 deferred 1건·대상 비지 않음이라 **교집합 축만 단독 단언**하는 설계도 건전.
- **AC#8 ⚠️ 조건부**: RIG-3 단독 FAIL 로 원 결함 폐쇄. 잔여 = **O-24**(`@SuppressWarnings("static")` → `<clinit>` 오표기, `main` 은 `<init>` 정답) → AC#11.
- **무회귀 ✅**: 192/192 · mutation **PASS(332)** · 결정론 md5 3회 · 전 입력 exit **0/2 뿐** · Python 골든패스 **113건 문자 단위 동일** · `diff --check` clean. **Codex 신고와 전부 일치(과장 없음)**.
- **🔴 R-1 (차단 · 신규) — R-3: 교집합 축은 양쪽이 해소될 때만 건전**: sink 호출지점이 미해소면 막다른 길이 대상(`Task.exec`)이 아니라 **caller(`Flow.sink`)** 로 기록돼 교집합이 **구조적으로 공집합** ⇒ 조용히 드롭. **fresh `main` 2 → 브랜치 0**: **G1** `tasks.get(0).exec()` · **G2** `get().exec()` · **H1** `hops:1` 경계. **대조군 G3(파라미터)·G4(지역변수) 는 유지** ⇒ 레지스트리/디스패처 표준형만 정확히 뚫린다.
- **🔴 R-2 (차단 · 신규) — R-4: 해소된 인자전달 람다는 deferred 레코드 자체가 사라진다**: 필드 초기화 경로는 레코드를 남기는데 **메서드 본문 경로(`:700-707`)는 해소 시 엣지만 만들고 레코드 없음**. **fresh G6**(`Registry.put` 등록 + `reg.find(k).exec()`) → 브랜치 `pass`·**coverage 0건·`fail_closed: []`·`errors: []`** = 흔적조차 없는 통과(R-3 보다 나쁨).
- **🔧 보정 방향(탐지·소음 동시 실측 — A-0050 §4 약속 이행)**: (a) 불투명(=`kind: unresolved` caller 유래) 막다른 길이 있으면 교집합 정밀화 미적용 → R-3 폐쇄 · (b) 불투명 호출지점 → 람다구현 `bodyless` 노드 보수 인접 → R-4 폐쇄. **G1·G2·H1·G6 전부 복구 · F6 `pass` 유지 · N3(무관 변경) `pass` 유지 = `main` 회귀 아님 · 스위트 192/192 무변화 · 결정론 유지 · 원복 확인.** (a)·(b) 는 서로 다른 경로를 덮어 **둘 다 필요**. 대가는 N1 소음 복귀(= O-14 미폐쇄, 의도된 후퇴).
- **하류 정직성**: (b) 는 관측 안 된 홉을 넣으므로 카드가 실경로처럼 렌더한다 ⇒ **보수 추정 홉 표식 필수**(AC#10).
- **🔴 자기정정(5회 연속)**: R-3 은 A-0050 이 교집합 축을 **F6(양쪽 해소) 하나로만** 확인한 탓, R-4 는 `invocation_parameter_type` 을 **RIG-D 단독 FAIL 만으로** 통과시킨 탓. ⇒ **rig 는 *추가된* 단언만 검증하고 *사라진* 커버리지는 검증하지 못한다. 판별식을 좁히는 변경은 좁힘의 여집합을 fresh 입력으로 반드시 친다.** A-0047→A-0051 5회 모두 같은 자리.
...
## 2026-07-22 — TASK-041 A-0055 보정 재제출 재리뷰 (`9716fb5`·`5b32acc`) → 🔴 보정요청 · 머지 보류 (A-0056 · D-100)

- **범위(멱등성)**: `1bb7972..5b32acc` 델타. `1f0c60b`·`0ed3c07`(D-098) · `52e59d4`·`1bb7972`(D-099) 는 처리 완료 — 재리뷰 안 함. 실측은 `main`(9bef1fc) ↔ 브랜치 기준, fresh 입력은 전부 `tests/fixtures/` 밖.
- **✅ AC#11**: `record_has_type_evidence` 보수 승격 — **RIG-F**(승격 분기 1줄 제거) **209/213, 신규 3건 단독 FAIL**.
- **✅ AC#12**: `type_supers` + 전이 폐포 — **RIG-G**(폐포 호출 제거) **210/213, 신규 2건 단독 FAIL**. fresh **A5**(2단 상속)로 전이성 확인.
- **✅ AC#13**: 브랜치 **212/213** ↔ `main` **201/202**(유일 FAIL `tree-sitter-smoke` = 파서 미설치, 양쪽 동일) · mutation **PASS(373)** · `PYTHONHASHSEED` 0/1/999/12345 md5 동일 · `parity 15/15` · `default` `main` 동일 · 전 입력 exit **0/2 뿐** · `py_compile`·`diff --check` PASS. **Codex 신고 그대로 재현 — 과장 없음.**
- **🔴 R-5 (차단 · 흔적 없는 과소탐)**: 증거 판정이 **caller 단위 스캔**이라 배선 메서드의 **무관한 호출 한 줄**이 탐지를 끈다. **신규 픽스처 `…no-type-jdk` 는 소음이 `sink()` 에 있어 이 갈래에 도달조차 못한다 = false-confidence.** **fresh B1** = 그 픽스처에서 **소음을 `wire()` 로 옮기기만** 함 → `main` 2 → 브랜치 **0**(완전 무흔적). B2(`StringBuilder`)·B3(2단 위임)·A2(같은 줄)도 동일. 대조군 A1·A3·A5·B4·B6 생존 + 소음 폐쇄 A7 생존.
- **🔴 R-6 (차단 · 흔적 없는 과소탐)**: 폐포 **단방향**. **A5(하위→상위)는 잡고 A4(상위→하위)는 놓친다** — `Runnable` 필드를 `Task` 로 좁혀 dispatch. B5(형제 캐스트)도 동일. `main` 2 → 브랜치 0.
- **⚠️ 리뷰어 자기정정 8회째**: **R-5 는 내 처방의 결함**이다 — A-0055 §3(a) 에서 증거의 마지막 갈래를 "같은 caller 의 unresolved" 로 못박았고 Codex 는 정확히 구현했다. R-1 이 지적한 *"무관한 한 줄이 판정을 뒤집는다"* 를 **내가 새 자리에 재생산**했다. **교훈: 보수 승격의 "증거" 를 정의할 땐 그 증거가 레코드와 실제로 결부되는지(범위)부터 못박아라 — caller 단위 스캔은 언제나 잡음을 삼킨다.**
- **🔧 보정 방향(`wt-fix-r6` 실측·원복 확인)**: (c) 증거를 **레코드-결부**로 한정(추출기가 감싼 invocation 을 레코드에 부여 권고) · (d) 폐포 **양방향**. **(c)+(d) → B1·B2·B3·A4·B5 5형태 복구 · A7(N4) `pass` 유지(O-14 폐쇄 생존) · 대조군 불변 · 스위트 212/213 회귀 0.** **단 A2(같은 줄 소음) 잔존 — 라인 근사는 임시일 뿐, AC#14 는 A2 까지 요구.**
- **🟡 O-30 격상 정정**: A-0055 가 "판정 무영향" 으로 적은 `List<String>`→`String` 누출이 **B1 에서 탐지를 끄는 바로 그 값**이다. **🟡 O-31 신규**: 소음 폐쇄가 줄 배치에 민감(과대 방향 ⇒ 비차단).
- **§1 통과**: `kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md`·`TASKS.md`·Claude 소유 **0 바이트**. 케이스 차집합 = 추가 5·제거 0. 무관 리팩터·기대값 약화 0. 커밋 §3 5절 2건 완비. **`type_supers` 는 소비자 0건인 순수 추가 키.** **잔티: `origin/main` 미동기화 재제출 11회째.**
- **★★ 킷 심층리뷰(형 지시) — 해당 없음**: `kit/` 0 바이트 + 현행 킷은 `extract-java-callgraph.py` **미탑재**·`kit/gates/check-indirect-impact.py` 에 Java L3 심볼 **0건** ⇒ 미탑재(구멍 아님). **TASK-038 sync 금지 유지.**
- **머지**: ❌ 코드 브랜치 보류. 리뷰 기록만 `main` 머지(D-007). 민감 변경 아님 ⇒ H-XXXX 불필요.

## TASK-041 A-0056 보정 재제출 재리뷰 (`a59d819`·`ecd906a`) — 🔴 보정요청 (D-101 · A-0057) — 2026-07-22
- **범위(멱등성)**: `5b32acc..ecd906a` 델타만. D-098·D-099·D-100 처리분은 재리뷰 안 함. 격리 worktree 4개(`wt-main-r7`=2f525d6 / `wt-prev-r7`=5b32acc / `wt-br-r7`=ecd906a / `wt-fix-r7`=프로토타입), fresh 입력 **21형태 전부 `tests/fixtures/` 밖**(rigged 차단).
- **✅ AC#14·#15·#16 폐쇄 — 음성검증으로 load-bearing 확인**: **RIG-K**(증거 판정을 옛 caller 스캔으로 원복) → **214/218, 신규 3건 단독 FAIL** / **RIG-I**(`opaque_receiver_types` 폐포 4줄 제거) → **215/218, 신규 2건 단독 FAIL**. **A2(같은 줄 소음)도 실제로 닫힘** — 신규 픽스처 3건이 소음을 람다와 같은 줄에 배치, fresh A2·B1·C10 복구. **A-0056 이 지적한 false-confidence 픽스처 해소**(소음이 레코드의 caller 안에 위치). 부수 개선(Codex 미신고): 새 dedup rank 는 전순서 최소값이라 `set` 순회 의존이던 옛 규칙보다 **결정론적**.
- **🔴 R-7 (차단 · 순회귀 포함)**: 조상 순회가 "람다가 그 호출의 **인자**인가" 를 묻지 않고 **대입식에서 끊지도 않아**, `d.task = () -> …` 를 감싼 무관한 호출이 타입 증거가 된다. `precise_receiver_type(object=None) → simple_name(owner)` 가 증폭. **C2**(`void keep(Object o){} … keep(대입식)`) `main` 2 · **직전 2** · 브랜치 **0 = 순회귀**(해소되는 호출이라 옛 스캔엔 증거 없었음 ⇒ **없던 증거를 만들어냄**) · **C1**(`Objects.requireNonNull(대입식)`) 0 · **C6**(`rows.add(String.valueOf(대입식))`) 0. **게이트는 C2 와 N4 를 구분할 수단이 없다**(둘 다 증거값 `Flow`).
- **🟠 R-8 (차단 · 1토큰)**: `rank = (bool(receiver), not bool(enclosing), …)` 가 **증거 있는 레코드를 선호** = 보수성 역방향. lambda dedup key 에 `line` 이 들어가 **같은 줄 두 람다가 합쳐지고**, **C3** 에서 소음 람다가 실제 등록 람다를 밀어내 `pass`. 정직히: C3 는 직전 대비 순회귀 아님(C7·C11 변형도 만들어 돌렸으나 전부 생존). 그래도 방향·비용상 차단.
- **⚠️ 리뷰어 자기정정 9회째**: R-7 은 **내 A-0056 AC#14 문언의 과소지정**("감싼 invocation" → Codex 는 "가장 가까운 조상 method_invocation" 으로 정확히 구현). **Codex 귀책 아님.** 교훈 = "결부" 를 요구할 땐 **문법적 형태**(인자 위치 · 바인딩 경계 절단)를 AC 에 명시하라.
- **A-0056 문언 정정**: 폐기 레코드는 *"완전 무흔적"* 이 아니라 `coverage.unevaluated` 에 **남는다**(C2 JSON 실측). 다만 항상 존재하는 원본 패스스루라 억제 신호가 아님 ⇒ 정확한 표현은 **"거버넌스 신호 부재"**. → O-33.
- **🔧 보정 방향(`wt-fix-r7` 실측·원복 확인)**: (e) 조상 순회에 ① 대입식/선언자에서 **중단** ② `method_invocation` 은 자식이 **`arguments` 일 때만** 채택 · (f) 동점규칙 `not bool(enclosing)` → `bool(enclosing)`. **(e)+(f) → C1·C2·C3·C6 복구 · A2·B1·A4·B5·C10 유지 · A7(N4) `pass` 유지(O-14 폐쇄 생존) · 대조군 9형태 불변 · 스위트 217/218 회귀 0.**
- **무회귀**: 브랜치 **217/218** ↔ `main` **201/202**(유일 FAIL `tree-sitter-smoke` = 파서 미설치, 양쪽 동일) · mutation **PASS(383)** · `PYTHONHASHSEED` 0/1/999/12345 md5 동일(신규 픽스처 3종) · exit 0/2 뿐(fresh 21 + 스위트) · `py_compile`·`diff --check` PASS · 케이스 차집합 = 추가 5·제거 0.
- **§1 통과**: Codex 저작 커밋이 만진 파일 = 게이트 2 · `cases.yaml` · 픽스처 5세트 · handoff · summaries **뿐**. `TASKS.md`·`CLAUDE.md`·`COMMON-RULES.md`·`PROJECT.md` **`main` 과 바이트 동일**(머지 유입) · `kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md` **0 바이트**. 무관 리팩터·기대값 약화 0. **`origin/main` 머지 이행(`57a0c74`) — 미동기화 11회 만에 해소.**
