...
## D-099 — TASK-041 A-0054 보정 재제출 재리뷰 — 🔴 **보정요청 · 코드 브랜치 머지 계속 보류** (AC#5~#10 전부 폐쇄 / R-4 신규 차단 — 타입 부재·불일치를 "무관"으로 단정하는 과소탐) (A-0055) — 2026-07-22

**대상**: `codex/2026-07-22-task041-java-l3-noise` — `52e59d4`(보정) · `1bb7972`(인계기록). **범위 = `0ed3c07..1bb7972` 델타**(`1f0c60b`·`0ed3c07` 은 D-098 처리 완료 — 재리뷰 없음). 판정 실측은 `main`(8cfb9e2) ↔ 브랜치 3-dot 기준, fresh 입력은 전부 `tests/fixtures/` **밖**에서 생성.

**결정**: **보정요청.** 코드 브랜치는 `main` 에 머지하지 않는다. 리뷰 기록(`decisions.md`·`review-notes.md`·`collab/answers/A-0055.md`·TASKS.md AC#11~#13)만 `main` 에 머지한다(D-007). **새 태스크 아님 — 같은 브랜치에 보정 커밋만 재제출.**

**✅ 닫힌 것(재작업 불필요) — AC#5~#10 전부.** Codex 는 A-0054 의 (a)~(e) 문언대로 정확히 구현했고 각 축을 rig 로 load-bearing 확인했다. **AC#5(a)** `precise_receiver_type()` 신설(체인 수신자 = `None`, `receiver_type()` 무개변) — **RIG-B**(원 헬퍼로 되돌림) **203/208, 4건 FAIL**. **AC#5(b)** 전칭→**존재(any)** 판정 — **RIG-A** **P4·P6·P7 3건 단독 FAIL** ⇒ **O-27(회귀보호 0) 폐쇄**. **AC#6** 이름 화이트리스트 **잔재 0**, `resolve` 명명 픽스처(P15)가 RIG-A/RIG-B 양쪽에서 FAIL ⇒ 이름 비의존 탐지 고정. **AC#7** 불투명 dead-end 상시 열거 + 빈 `dead_ends=` 금지 불변식 — **RIG-C** **193/208, 14건 FAIL**. **AC#8** 5필드 dedupe·미상 우선 — **RIG-D** **205/208, 2건 FAIL**; 추가로 **`PYTHONHASHSEED` 0/1/999/12345 변주 md5 동일**·전 픽스처 "동일 키 서로 다른 truthy 타입" 충돌 **0건**·인위적 충돌에서도 **미상 쪽 생존**(보수 방향) ⇒ set 순회 의존 없음. **AC#9** O-28 정정 정확. **AC#10** 무회귀. **Codex 신고와 전부 일치 — 과장 없음.**

**🔴 R-4 (차단 · 흔적조차 없는 과소탐 · `main` 보다 나쁨) — 타입 "부재/불일치" 를 "무관" 으로 단정한다.** 존재 판정이 꺼지면 승격은 오로지 `type_match = opaque_receiver_types ∩ deferred_types` 에 달리는데, 이건 **양쪽에서 독립적·불완전하게 추정된 타입 이름의 문자열 동등비교**다. **근거가 없거나 어긋나면 레코드를 통째로 버린다** — `errors:[]`·`fail_closed:[]`·`indirect_impact:[]` **완전 무흔적**. **fresh 실증(픽스처 밖 · `main` 2 → 브랜치 0)**: **Q20** `class Dispatcher{ Runnable task; void fire(){ task.run(); } }` + `wire(){ d.task = () -> new Vault().transfer(); }` + `sink(){ d.fire(); }` — **JDK 호출이 한 줄도 없다**(R-1 의 잔여가 아니라 별개 갈래) · **Q12**(Q20+`rows.size()`) · **Q18**(2단 위임) — 셋 다 deferred 레코드의 **타입 근거가 0**(`deferred_types=∅`) · **Q11b/Q11**(`interface Task extends Runnable`, `Task` 로 등록 → `Runnable` 로 dispatch) — 타입은 **양쪽 다 정확한데 상속관계 미모델링**으로 불일치. **대조군은 전부 생존**(Q1·Q2·Q4·Q5·Q6·Q7·Q8·Q10·Q13·Q15·Q17 = `main` 과 동일 `approval_required`) **+ 소음 폐쇄도 생존**(N4·Q9 `pass`) ⇒ 탐지가 통째로 죽은 게 아니라 **위임/서브인터페이스 관용구만 정확히 뚫린다**. `A.field = () -> …` 로 남의 객체에 콜백을 꽂고 그 객체가 위임 발사하는 형태는 **리스너/핸들러/디스패처의 가장 흔한 Java 관용구**다. **§2B 필수질문 = 예** ⇒ 비차단 금지.

**⚠️ 리뷰어 자기정정 7회째** — R-4 는 이번 보정 델타가 만든 게 아니라 **AC#1(O-14) 정밀화의 잔여**로 `1f0c60b` 시점부터 있었고(`0ed3c07` 에서 직접 재현), **A-0054 에서 내가 all/any 축만 rig 하고 놓쳤다**. Codex 귀책 아님. 그러나 **브랜치 전체가 아직 `main` 에 없으므로 지금 막는다**(A-0051 R-3·R-4 선례).

**🔧 보정 방향 — 리뷰어가 워크트리에 직접 패치해 탐지축·소음축을 동시 실측**(A-0050 §4 절차). **(a)** 타입 근거가 **아예 없는** deferred 레코드는 보수 승격 = **R-1 의 거울상**(불투명 호출 쪽에서 전칭→존재로 고친 오류가 deferred 쪽에 남아 있다). `record_has_type_evidence()` = `dispatch_targets` ∨ `receiver_type` ∨ `anonymous_class "new X"` ∨ **같은 caller 의 unresolved 가 타입을 갖는지** — 마지막 갈래가 **N4 를 지킨다**(N4 는 `Flow.wire` 에 `consume`(`receiver_type: Flow`) 이 있어 근거 있음 → `pass` 유지 / Q20 은 `Flow.wire` 에 unresolved 가 하나도 없어 근거 0 → 승격). **(b)** 추출기가 이미 계산하는 `declaration_supertypes()` 를 JSON(`type_supers`)에 실어 `deferred_types` 를 **전이 폐포**로 확장. **(a)·(b) 둘 다 필요**(각각 단독 측정 확인 — (a)만으로는 Q11 계열이 안 고쳐진다). **측정표(원복 확인함)**: (a)+(b) 적용 시 **Q20·Q12·Q18·Q11·Q11b 5형태 전부 `main` 과 동일 복구** · **N4·Q9 `pass` 유지 ⇒ O-14 소음 폐쇄가 그대로 살아 있다** · 대조군 11형태 불변 · **스위트 207/208 유지(회귀 0)** ⇒ 소음↔탐지 트레이드오프가 아니라 **순수 개선**.

**무회귀·§1 보수적 개발 — 통과.** 브랜치 **207/208** ↔ `main` **201/202**(유일 FAIL `tree-sitter-smoke` 은 파서 미설치 · **양쪽 동일** ⇒ 브랜치 귀책 아님) · 케이스 집합 차집합 = 추가 7 / 제거 1(`…-noise-fail-closed`→`…-noise-pass` **개명**, D-098 수용분) · mutation **PASS** · `py_compile`·`git diff --check` PASS · **parity 15/15** · `default` 120/121 = `main` 동일(**Python 골든패스 무접촉**) · 전 입력 exit **`0`/`2` 뿐**. 3-dot diff 에 `kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md`·`TASKS.md`·Claude 소유 **0 바이트**, 무관 리팩터·기대값 약화 **0**, 커밋 §3 5절 2건 모두 완비.

**🟡 신규 비차단**: **O-29** `precise_receiver_type()` 의 `this.` 갈래가 `split(".",2)[1]` 로 두 번째 토큰만 봐서 `this.holder.task.run()` 을 **`Holder`** 로 확신(접두 없는 `holder.task.run()` 은 `None` 으로 정확 — **`this.` 하나로 갈린다**). 현재 판정 무영향(fresh Q1 확인)이나 **틀린 타입을 확신하는 자리** — AC#12 폐포가 들어오면 성질이 바뀔 수 있다. 폐쇄 방향: `this.` 뒤 토큰이 하나뿐일 때만 조회, 체인이면 `None`. **O-30** `List<String> rows` 바인딩이 **타입 인자** `String` 으로 누출돼 `rows.size()` 의 `receiver_type` 이 `String`(N4 기대값에 박혀 있다) — 판정 무영향이나 증적 사실성 문제.

**정책 결론**: `java.layers.callgraph` **`partial` 유지.** R-4 로 위임 관용구가 뚫려 있어 `supported` 는 under-claim 원칙 위반(D-074 O-5·D-075·D-076·D-097·D-098). 승격 조건 = **AC#1+#5+#6+#7+#11+#12** + O-19 폐쇄.

**킷**: 이번 브랜치도 킷 무접촉이라 TASK-038 sync 차단 해제 상태는 유지되나, **TASK-041 머지 전 sync 금지** 지시는 **강화 유지** — 지금 스냅샷하면 **R-4(위임 관용구 과소탐)를 배포 킷에 싣는다**(현행 킷은 Java L3 **미탑재** = 구멍 아님).

**비차단 이월**: O-29 · O-30 · O-22(구조적 근사 · 과대열거 방향) · O-13 · O-3 · O-15 · O-16. **~~O-27~~·~~O-28~~ 폐쇄.**

**상세**: `collab/answers/A-0055.md`.

## D-100 — TASK-041 A-0055 보정 재제출 재리뷰 — 🔴 **보정요청 · 코드 브랜치 머지 계속 보류** (AC#11·#12·#13 기능 충족·load-bearing / R-5·R-6 신규 차단 — 증거판정 과대범위 + 단방향 폐포) (A-0056) — 2026-07-22

**대상**: `codex/2026-07-22-task041-java-l3-noise` — `9716fb5`(보정) · `5b32acc`(인계기록). **범위 = `1bb7972..5b32acc` 델타**(`1f0c60b`·`0ed3c07` = D-098, `52e59d4`·`1bb7972` = D-099 처리 완료 — 재리뷰 없음). 판정 실측은 `main`(9bef1fc) ↔ 브랜치 기준, fresh 입력은 전부 `tests/fixtures/` **밖**.

**결정**: **보정요청.** 코드 브랜치는 `main` 에 머지하지 않는다. 리뷰 기록(`decisions.md`·`review-notes.md`·`collab/answers/A-0056.md`·TASKS.md AC#14~#16)만 `main` 에 머지한다(D-007). **새 태스크 아님 — 같은 브랜치에 보정 커밋만 재제출.**

**✅ 구현은 A-0055 문언대로 정확 · 두 축 모두 load-bearing.** **AC#11** `record_has_type_evidence` 보수 승격 — **RIG-F**(승격 분기 1줄 제거) **209/213, 신규 3건 단독 FAIL**. **AC#12** `type_supers` + 전이 폐포 — **RIG-G**(폐포 호출 제거) **210/213, 신규 2건 단독 FAIL**. 폐포가 실제로 **전이**함도 fresh **A5**(`Task→Mid→Runnable` 2단)로 확인. **AC#13** 무회귀. **Codex 신고와 전부 일치 — 과장 없음**(212/213 · adversarial 65/65 · negative-corpus 9/9 · mutation PASS(373) · `PYTHONHASHSEED` 4종 md5 동일 전부 재현).

**🔴 R-5 (차단 · 흔적 없는 과소탐) — 타입 증거 판정이 "같은 caller 의 아무 호출" 로 켜진다.** `unresolved_by_caller[caller]` 는 그 메서드 안 **모든** 미해소 호출이라, 배선 메서드에 **무관한 외부 호출 한 줄**만 있으면 증거 있음 → 승격 분기 사망 → 빈 `type_match` 로 떨어져 레코드 전량 폐기(`errors:[]`·`fail_closed:[]`·`indirect_impact:[]` **완전 무흔적**). **🔴 신규 픽스처가 이 갈래에 구조적으로 도달하지 못한다** — `…deferred-no-type-jdk` 는 소음 `rows.size()` 가 **`sink()`** 에 있는데 증거 판정은 **레코드의 caller = `Flow.wire`** 만 본다 ⇒ **소음 내성을 전혀 증명하지 못하는 false-confidence 픽스처**. **fresh 실증(`main` 2 → 브랜치 0)**: **B1** = 그 픽스처에서 **소음을 `sink()`→`wire()` 로 옮기기만** 함(의미상 동일 프로그램) · **B2**(`StringBuilder` 소음) · **B3**(2단 위임 + 소음) · **A2**(같은 줄 소음). 실전 `wire()`/`init()`/`configure()` 는 로깅·널체크가 거의 항상 섞이므로 **신규 3픽스처가 커버한다는 관용구가 현실 코드에선 대부분 다시 놓친다.** 대조군 전부 생존(A1·A3·A5·B4·B6) + 소음 폐쇄 생존(A7=N4 `pass`). **§2B 필수질문 = 예** ⇒ 비차단 금지.

**⚠️ 리뷰어 자기정정 8회째 — R-5 는 내 처방의 결함이다.** A-0055 §3(a) 에서 내가 마지막 갈래를 **"같은 caller 의 unresolved 가 타입을 갖는지"** 로 못박았고 Codex 는 그 문언을 정확히 구현했다. **Codex 귀책 아님.** R-1 이 지적한 *"무관한 한 줄이 판정을 뒤집는다"* 를 **내가 새 자리에 그대로 재생산**했다. **교훈: 보수 승격의 "증거" 를 정의할 땐 그 증거가 레코드와 실제로 결부되는지(범위)를 먼저 못박아라 — caller 단위 스캔은 언제나 무관한 잡음을 삼킨다.**

**🔴 R-6 (차단 · 흔적 없는 과소탐) — 폐포가 단방향.** AC#12 는 `deferred_types` 만 상위로 확장해 **"필드는 상위타입 선언 · 사용처에서 좁힘"** 이 뚫린다. **fresh A4**(`Runnable task` → `Task t = (Task) task; t.run()`, `interface Task extends Runnable`) · **B5**(형제 캐스트) = `main` 2 → 브랜치 **0**. **A5(하위→상위)는 잡고 A4(상위→하위)는 놓친다** — 같은 상속 한 쌍인데 폐포를 어느 쪽에 걸었느냐로만 갈린다. `B4`(인라인 캐스트)가 잡히는 건 수신자가 미상으로 떨어지는 **다른 경로** 덕이라 반증이 아니다.

**🔧 보정 방향(리뷰어가 `wt-fix-r6` 에 직접 패치해 실측 · 원복 확인함)**: **(c)** 증거를 **레코드-결부**로 한정(권고: 추출기가 감싼 invocation 을 레코드에 부여 — 게이트의 caller 역추측 제거). 실증 근거: N4 는 `consume`(line 9)이 lambda(line 9)와 **같은 줄** = 진짜 결부 / B1 은 `rows.size()`(line 12) vs lambda(line 13) = 무관. **(d)** 폐포를 **양방향**(과잉매칭은 과탐 방향 = MVP-3 계약상 안전). **측정표**: (c)+(d) 로 **B1·B2·B3·A4·B5 5형태 복구** · **A7(N4) `pass` 유지 ⇒ O-14 소음 폐쇄 생존** · 대조군 5형태 불변 · **스위트 212/213 유지(회귀 0)** ⇒ 순수 개선. **단 A2(같은 줄 소음)는 리뷰어 프로토타입에서도 잔존** — 라인 일치는 임시 근사일 뿐이며 **A2 까지 닫아야 한다.**

**무회귀·§1 보수적 개발 — 통과.** 브랜치 **212/213** ↔ `main` **201/202**(유일 FAIL `tree-sitter-smoke` = 파서 미설치, **양쪽 동일** ⇒ 브랜치 귀책 아님) · 케이스 차집합 = **추가 5건뿐, 제거 0** · mutation **PASS(373)** · `parity 15/15` · `default` 120/121 = `main` 동일(**Python 골든패스 무접촉**) · 전 입력 exit **`0`/`2` 뿐** · **`type_supers` 는 순수 추가 키로 `main` 전체에 소비자 0건**(하위호환 안전). 3-dot diff 에 `kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md`·`TASKS.md`·Claude 소유 **0 바이트**, 무관 리팩터·기대값 약화 **0**, 커밋 §3 5절 2건 모두 완비. **handoff·summaries 문언 정확 — 과장 없음**(오히려 *"같은 caller 에 타입 증거가 있는 N4 소음은 pass 유지"* 로 결함 조건을 정직하게 명시해 두었다).

**★★ 킷 심층리뷰(형 지시) — 해당 없음, 근거 실증.** 3-dot diff `kit/` **0 바이트**. 추가 확인: 현행 킷은 `extract-java-callgraph.py` **미탑재**(`kit/gates/` 21개 목록에 부재), `kit/gates/check-indirect-impact.py` 에 `java_deferred`·`record_has_type_evidence`·`type_supers`·`opaque_untyped` **0건** ⇒ **킷은 Java L3 미탑재 = 구멍이 아니라 미탑재.** run.sh 배선·verdict 조립·`--policies`·selftest 는 이 델타의 영향권 밖.

**🟡 신규/격상 비차단**: **O-30 격상 정정** — A-0055 가 *"판정 무영향"* 으로 적은 `List<String>`→`String` 타입인자 누출은 **이제 판정을 좌우한다**(B1 에서 바로 그 잘못된 `String` 이 "증거 있음" 을 켜서 탐지를 끈다). **O-31 신규** — 소음 폐쇄가 **줄 배치에 민감**(N4 형태 한 줄 = `pass`, 소음을 별도 줄로 = `approval_required`). 과대 방향이라 §2B 필수질문 = 아니오 ⇒ 비차단이나 증적 일관성 문제. **O-29 이월** — AC#15 양방향 폐포가 들어오면 틀린 타입이 더 넓게 퍼질 수 있어 함께 검토 권고.

**정책 결론**: `java.layers.callgraph` **`partial` 유지.** 승격 조건 = **AC#1+#5+#6+#7+#11+#12+#14+#15** + O-19 폐쇄.

**킷**: **TASK-041 머지 전 sync 금지** 유지·강화 — 지금 스냅샷하면 R-5·R-6 를 배포 킷에 싣는다.

**비차단 이월**: O-29 · O-30(격상) · O-31 · O-22 · O-13 · O-3 · O-15 · O-16.

**상세**: `collab/answers/A-0056.md`.

---

## D-101 — TASK-041 A-0056 보정 재제출 재리뷰 — 🔴 **보정요청 · 코드 브랜치 머지 계속 보류** (AC#14·#15·#16 기능 충족·load-bearing / R-7·R-8 신규 차단 — 결부 범위 과대 + dedup 동점규칙 역방향) (A-0057) — 2026-07-22

**대상**: `codex/2026-07-22-task041-java-l3-noise` — `a59d819`(보정) · `ecd906a`(인계기록) · `57a0c74`(`origin/main` 머지).
**범위(멱등성)**: **`5b32acc..ecd906a` 델타만.** `1f0c60b`·`0ed3c07`(D-098) · `52e59d4`·`1bb7972`(D-099) · `9716fb5`·`5b32acc`(D-100) 은 처리 완료 — 재리뷰 안 했다.

**✅ 닫힌 것(되돌리지 말 것) — AC#14·#15·#16.** 두 축 모두 rig 로 load-bearing 확인: **AC#14**(증거를 caller 역추측 → 추출기가 감싼 invocation 을 레코드에 부여) — **RIG-K**(옛 caller 스캔으로 원복) **214/218, 신규 3건 단독 FAIL**. **AC#15**(`opaque_receiver_types` 상위 폐포) — **RIG-I**(4줄 제거) **215/218, 신규 2건 단독 FAIL**. **A-0056 이 요구한 `A2`(같은 줄 소음)도 실제로 닫혔다** — 신규 픽스처 3건이 소음을 람다와 **같은 줄**에 두었고 fresh A2·B1·C10 복구. **A-0056 이 지적한 false-confidence 픽스처 문제도 해소**(소음이 이제 레코드의 caller 안에 있어 증거 경로를 실제 통과). **무회귀** — 브랜치 **217/218** ↔ `main` **201/202**(유일 FAIL `tree-sitter-smoke` 은 파서 미설치 · 양쪽 동일) · mutation **PASS(383)** · `PYTHONHASHSEED` 0/1/999/12345 md5 동일 · `parity 15/15` · `default` 120/121 `main` 동일 · 전 입력 exit **0/2 뿐** · `py_compile`·`diff --check` PASS · 케이스 차집합 = 추가 5·제거 0. **`origin/main` 머지 이행 — 미동기화 11회 만에 해소.** **Codex 신고와 전부 일치 — 과장 없음.** 부수 개선(미신고): 새 dedup rank 는 전순서 최소값이라 **`set` 순회 의존이던 옛 규칙보다 결정론적**이다.

**🔴 R-7 (차단 · 과소탐 · 직전 커밋 대비 순회귀 포함) — 결부 범위가 여전히 너무 넓다.** 조상 순회가 **"람다가 그 호출의 인자인가"** 를 묻지 않고 **대입식에서 끊지도 않아**, `d.task = () -> …` 를 감싼 **무관한 호출**이 그대로 타입 증거가 된다. `precise_receiver_type(object=None) → simple_name(owner)`(자기 클래스명)가 이를 증폭한다. fresh(전부 픽스처 밖): **C2** `void keep(Object o){} … keep(d.task = () -> …)` — `main` 2 · **직전 5b32acc 2** · 브랜치 **0 🔴순회귀**(`keep` 은 해소되는 호출이라 옛 caller 스캔엔 증거가 없었다 = **없던 증거를 만들어낸다**) · **C1** `Objects.requireNonNull(대입식)` **0** · **C6** `rows.add(String.valueOf(대입식))` **0**(O-30 타입인자 누출이 다시 판정을 좌우). **게이트는 C2 와 N4 를 구분할 수단이 없다** — 둘 다 증거값이 `Flow` 이고, 유일한 실질 차이는 **람다가 인자인가(N4) / 호출이 대입식을 감쌌을 뿐인가(C2)** 다.

**🟠 R-8 (차단 · 1토큰 · 보수 방향 역전) — dedup 동점규칙이 증거 있는 레코드를 우선.** `rank = (bool(receiver), not bool(enclosing), …)` 는 **`enclosing` 있는 쪽을 선호** — `enclosing` 은 탐지를 끄는 값이라 방향이 거꾸로다. lambda dedup key 가 `(caller, kind, "lambda", line, dispatch_targets)` 라 **같은 줄 두 람다가 한 레코드로 합쳐지고**, fresh **C3**(`Objects.requireNonNull(() -> hashCode()); d.task = () -> …;`)에서 **소음 람다가 실제 등록 람다를 밀어내** `pass`. **정직하게: C3 는 직전 대비 순회귀가 아니고**(5b32acc 도 `pass`), 순회귀를 만드는 변형(C7·C11)도 직접 만들어 돌렸으나 **전부 생존**. 그래도 차단 — 방향이 명백히 보수성 반대이고 **수정이 `not` 두 개 제거뿐**이며 R-7 수정 후 `enclosing` 영향력이 커진다.

**⚠️ 리뷰어 자기정정 9회째 — R-7 은 내 처방 문언의 과소지정이다.** A-0056 AC#14 는 *"이 레코드를 감싼 invocation"* 이라고만 썼고 Codex 는 **"가장 가까운 조상 `method_invocation`"** 으로 문언대로 구현했다. 내가 뜻한 것은 *"람다가 **인자로 넘어간** 그 호출"* 이었다. **Codex 귀책 아님.** **교훈: "결부" 를 요구할 땐 결부의 *문법적 형태*(인자 위치 · 바인딩 경계에서 절단)를 AC 에 명시하라 — "감쌌다" 는 대입식 바깥까지 삼킨다.**

**A-0056 문언 정정**: 폐기 레코드가 *"완전 무흔적"* 이라고 적었으나 **부정확** — `coverage.unevaluated` 에는 남는다(C2 JSON 실측). 다만 그건 모든 실행에 항상 있는 추출기 원본 패스스루라 억제 신호가 아니다. 정확한 표현은 **"거버넌스 신호 부재"**(`verdict: pass`·`exit_code: 0`·`errors:[]`·`fail_closed:[]`). 판정 결론은 불변 → **O-33** 으로 별도 관리.

**🔧 보정 방향(리뷰어가 `wt-fix-r7` 에 직접 패치해 실측)**: **(e)** 조상 순회에 ① `assignment_expression`/`variable_declarator` 만나면 **중단** ② `method_invocation` 은 **올라온 자식이 `arguments` 일 때만** 채택. **(f)** 동점규칙 `not bool(enclosing)` → `bool(enclosing)`. **측정표**: (e)+(f) 로 **C1·C2·C3·C6 4형태 복구** · **A2·B1·A4·B5·C10 유지** · **A7(N4) `pass` 유지 ⇒ O-14 소음 폐쇄 생존** · 대조군 9형태 불변 · **스위트 217/218 유지(회귀 0)** ⇒ 순수 개선.

**무회귀·§1 보수적 개발 — 통과.** Codex 저작 커밋이 만진 파일 = 게이트 2개 · `cases.yaml` · 픽스처 5세트 · handoff · summaries **뿐**. `TASKS.md`·`CLAUDE.md`·`COMMON-RULES.md`·`PROJECT.md` 는 `main` 과 **바이트 동일**(머지 유입). `kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md` **0 바이트**. 무관 리팩터·기대값 약화 **0**, 커밋 §3 5절 완비. `enclosing_invocation_receiver_type` 은 소비자 2곳뿐인 순수 추가 키(하위호환 안전).

**★★ 킷 심층리뷰(형 지시) — 해당 없음, 근거 실증.** 3-dot diff `kit/` **0 바이트**. 재실측: 킷 `kit/gates/` **22개**에 `extract-java-callgraph.py` **부재**, `kit/gates/check-indirect-impact.py` 에 `java_deferred` **0건** ⇒ **미탑재 = 구멍 아님.** run.sh 배선·verdict 조립·co-located 의존·`HAS_RANGE`/정책부재/`--policies`/fail-closed·sync·selftest 는 **이 델타의 영향권 밖**. **TASK-038 sync 는 R-7·R-8 폐쇄 전까지 계속 금지.**

**🟡 신규 비차단**: **O-32** `nested_in_deferred_body()` 가 지연 본문 안 중첩 람다를 순회에서 제외 — fresh **C4**(`Optional.ifPresent(c -> { d.task = () -> … })`) `main` 2 → 직전 0 → 브랜치 0 → 프로토타입 0. **이 델타의 회귀 아님**이고 리포 내 고차함수(C4b)·익명클래스(C4c) 형태는 4개 트리 전부 탐지 ⇒ 범위가 *"JDK/미해소 고차 호출의 콜백 안 등록"* 으로 좁다. 닫으려면 N4 소음 폐쇄와 정면충돌 ⇒ 패치가 아니라 설계 판단 ⇒ **`supported` 승격 차단조건**으로 못박음. **O-33** 폐기된 deferred 레코드에 **거버넌스 신호가 없다** — R-1·R-4·R-5·R-7 이 매번 조용히 통과한 근본 원인. 억제 사유를 1급 표기하면 다음 과소탐이 리뷰가 아니라 **출력**에서 드러난다.

**정책 결론**: `java.layers.callgraph` **`partial` 유지.** 승격 조건 = **AC#1+#5+#6+#7+#11+#12+#14+#15+#17+#18** + **O-19·O-32** 폐쇄.

**비차단 이월**: O-29 · O-30(격상 유지) · O-31(부분해소) · O-32(신규) · O-33(신규) · O-22 · O-13 · O-3 · O-15 · O-16.

**상세**: `collab/answers/A-0057.md`.

---

## D-102 — TASK-041 A-0057 보정 재제출 재리뷰 — 🔴 **보정요청 · 코드 브랜치 머지 계속 보류** (AC#18·#17① 기능 충족·load-bearing / AC#17② 무효 판정 — 리뷰어 처방 결함 / R-9 신규 차단 — 결부 경계가 단방향) (A-0058) — 2026-07-22

**대상**: `codex/2026-07-22-task041-java-l3-noise` — `0aa6d11`(보정) · `67723f9`(인계기록).
**범위(멱등성)**: **`ecd906a..67723f9` 델타만.** `1f0c60b`·`0ed3c07`(D-098) · `52e59d4`·`1bb7972`(D-099) · `9716fb5`·`5b32acc`(D-100) · `a59d819`·`ecd906a`(D-101) 은 처리 완료 — 재리뷰 안 했다.

**✅ 닫힌 것(되돌리지 말 것) — AC#18 · AC#17①.** **AC#18**(dedup 동점규칙 `not bool(enclosing)` → `bool(enclosing)`) — **RIG-C**(원복) **220/222, `…same-line-lambda-dedup` 단독 FAIL**; 픽스처의 `(Runnable)` 캐스트가 결과를 떠받치는지 의심해 **캐스트 없는 fresh C3** 로 독립검증 → `wt-br` 2 ↔ `rigC` 0 ⇒ 픽스처 정직. **AC#17①**(대입식/선언자에서 순회 중단) — **RIG-A**(`break` 2줄 제거) **218/222, 신규 3건 단독 FAIL**; fresh C2·C1·C6 로 **C2 순회귀 복구** 확인(`main` 2 · 직전 0 · 브랜치 2).

**⚠️ AC#17② 는 무효(inert) — 리뷰어 자기정정 10회째.** `child == arguments` 가드는 **RIG-B**(제거) 로 **FAIL 0건**, **RIG-D**(계측: `child != arguments` 인데 `precise_receiver_type` 이 non-None 인 순간 기록) 로 **스위트 전량 0회**. 원인: ① 이 대입/선언자에서 끊은 뒤 람다가 `arguments` 아닌 경로로 invocation 에 닿는 형태는 **`object` 필드(캐스트·괄호 경유)** 뿐인데 `precise_receiver_type()` 이 그 노드타입을 어느 갈래에도 태우지 않아 **항상 `None`**. **Codex 귀책 아님 — 불가능한 음성검증을 요구한 내 처방의 결함.** 가드는 무해한 방어코드로 **유지**, 회귀 픽스처 요구는 **철회**.

**🔴 R-9 (차단 · 과소탐 · `main` 대비 탐지 손실) — 결부 경계가 "위쪽" 으로는 없다.** AC#17① 은 람다→조상 방향만 끊는다. **채택한 invocation *위*에 대입·선언·`return` 이 있어도 `method_invocation` 에서 무조건 `break` 하므로 아무 일도 없다.** ⇒ **래핑 등록 관용구**가 통째로 뚫린다. 증거는 또다시 `precise_receiver_type(None) → simple_name(owner)` = 자기 클래스명(`Flow`)이거나 무관 JDK 타입. **fresh 9형태(전부 픽스처 밖) `main` 2 → 브랜치 `pass`(0)**: **H2** `d.task = wrap(L)` · **H2c** `d.task = Objects.requireNonNull(L)` · **H2d** `Runnable r = wrap(L); d.task = r;` · **H2e** `d.task = svc.wrap(L)` · **H3** `return wrap(L)` 반환 경유 · **H7** 2중 래핑 · **H9** 캐스트 경유 · **K2** 배열 초기화 경유 · **K3** `svc.register(new Holder(L))`(생성자를 뚫고 바깥 호출 수신자를 가져오는 **R-7 오귀속 잔존**). **커밋별 이분탐색: 브랜치 첫 코드 커밋 `1f0c60b` 부터 이미 `pass` ⇒ 이 델타의 순회귀는 아니다. 그러나 `main` 은 9형태를 전부 잡는다 ⇒ 머지하면 `main` 이 지금 잡는 탐지를 잃는다.** 대조군(A1·A2·C10·C1·C2·C3·C6·H1c·H4·H5·H6·H10·K1)과 소음폐쇄(N4·N5·N6)는 전부 생존 ⇒ 구멍은 **"래퍼 호출을 거쳐 바인딩/반환되는 등록"** 한 갈래이며, 실전 Java 의 가장 흔한 등록 관용구(`disposable = obs.subscribe(…)`·`handle = bus.register(…)`)가 정확히 여기 든다. **§2B 필수질문 = 예**(실재 민감 경로가 `verdict: pass`·`exit 0`·`errors:[]`·`fail_closed:[]` 로 소멸) ⇒ **비차단 금지.** R-1·R-4·R-5·R-6·R-7 과 동일 계열, 전부 차단이었다.

**🔧 보정 = AC#20(결부 경계 양방향) + AC#21(무회귀).** 리뷰어 프로토타입(`wt-fix3` · 현행 대비 **4줄**): `method_invocation` 채택 후 **`break` 대신 `adopted` 플래그로 순회 계속**(채택은 1회로 한정 — R-7 재발 금지) + 경계집합에 **`return_statement`·`object_creation_expression` 추가** + 경계에서 **채택분까지 폐기**. 실측: **9형태 전부 복구 · 대조군 13형태 불변 · N4·N5·N6 `pass` 유지 · 스위트 221/222 회귀 0** ⇒ **소음↔탐지 트레이드오프가 아니라 순수 개선**. 중간 단계도 남김(`wt-fix` 6형태 / `wt-fix2` 8형태 / `wt-fix3` 9형태, 셋 다 221/222).

**무회귀·§1 보수적 개발 — 통과.** 브랜치 **221/222** ↔ `main` **201/202**(유일 FAIL `tree-sitter-smoke` = 환경 파서 의존성, 양쪽 동일) · adversarial 74/74 · default 120/121(`main` 동일 ⇒ **Python 골든패스 무접촉**) · parity 15/15 · 케이스 차집합 = 추가 4·제거 0 · mutation **PASS(391)** · `PYTHONHASHSEED` 0/1/999/12345 md5 동일(신규 픽스처 4종) · **전 입력 exit 0/2 뿐**(파싱 불능 소스도 fail-closed 2) · `py_compile`·`diff --check` PASS. Codex 저작 커밋이 만진 파일 = 게이트 1개(+13/−8)·`cases.yaml`(+60)·픽스처 4세트·handoff·summaries **뿐**. `TASKS.md` 는 머지베이스와 바이트 동일(브랜치 무접촉) · `kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md`·Claude 소유 **0 바이트**. 무관 리팩터·기대값 약화 **0**, 커밋 §3 5절 완비. **handoff·summaries 문언 정확 — 과장 없음**(221/222·74/74·391 전량 재현, `tree-sitter==0.26.0` 미확보 한계도 정직 고지). **`origin/main` 재머지 생략**은 미머지분이 내 리뷰기록 2커밋(docs only)뿐이라 **AC#19 위반으로 세지 않는다.**

**★★ 킷 심층리뷰(형 지시) — 해당 없음, 근거 실증.** 브랜치 전체 3-dot diff `kit/` **0 바이트**(numstat 행 0). 재실측: `kit/gates/*.py` **20개**에 `extract-java-callgraph.py` **부재**, 킷 전체에 `java_deferred`·`enclosing_invocation_receiver_type`·`record_has_type_evidence` **0건** ⇒ **미탑재 = 구멍 아님.** run.sh 배선·verdict 조립·co-located 의존·`HAS_RANGE`/정책부재/`--policies`/fail-closed·sync·selftest 는 **이 델타의 영향권 밖**. **TASK-038 sync 는 R-9 폐쇄 전까지 계속 금지.**

**🟡 신규 비차단**: **O-34** `precise_receiver_type(object_node=None) → simple_name(current_owner)` — 무자격 호출의 "수신자 타입" 을 **자기 클래스명**으로 단정하는데 이는 람다의 함수형 타입과 무관하다. **R-7·R-9 가 둘 다 이 값에서 증폭됐다.** AC#20 은 경계로 우회할 뿐 값 자체는 남는다 ⇒ 무자격 호출은 증거 `None`(보수)로 두거나 자기 클래스명을 후보에서 제외. **O-35(방법론)** AC 가 요구한 음성검증이 **원리적으로 불가능한 경우**가 있다(AC#17②) ⇒ rig 요구를 쓰기 전에 "그 갈래에 도달하며 값이 non-None 인 입력이 존재하는가" 를 **처방 단계에서 확인**하고, 이번에 쓴 **계측 리그(RIG-D) 패턴을 상설화**한다. **O-33 격상** — R-9 9형태가 **또 전부 조용히** 통과했다. R-1·R-4·R-5·R-7·R-9 다섯 계열이 모두 같은 무신호 경로로 샜으므로 **`supported` 승격 차단조건에 O-33 추가**.

**정책 결론**: `java.layers.callgraph` **`partial` 유지.** 승격 조건 = **AC#1+#5+#6+#7+#11+#12+#14+#15+#17①+#18+#20** + **O-19·O-32·O-33** 폐쇄.

**비차단 이월**: O-29 · O-30(격상 유지) · O-31(부분해소) · O-32 · O-33(격상) · O-34(신규) · O-35(신규) · O-22 · O-13 · O-3 · O-15 · O-16.

**상세**: `collab/answers/A-0058.md`.

## D-103 (2026-07-22) TASK-041 이월(DEFERRED) — Java L3 소음(O-14) 폐쇄는 tree-sitter 로 불가·정밀솔버 이후로 · 형 지시

**형 지시**: "41 이월하고 킷으로 가자. 왜 보정→보정 반복인지 Codex 가 배우게 명시해줘. 저거 고칠 수 있긴 하냐?"

**결정**: TASK-041 **이월**. 브랜치 `codex/2026-07-22-task041-java-l3-noise`(+11) **머지 안 함**(main 보다 나쁨=과소탐). 다음 = **TASK-038(킷)**.

**5라운드 보정 반복의 근본 원인(Codex 학습용)**:
- **41 목표** = Java 간접영향의 **과잉 경고(소음·O-14)** 감축 → 지연 dispatch 승격 판별식을 **좁힘**.
- **왜 무한 반복(D-098~102 · R-1~R-9 · 리뷰어 자기정정 10회)**: 판별식을 **좁히면 반드시 과소탐(실제 위험 놓침) 구멍**이 여집합에서 생긴다. 매 라운드 지난 구멍은 닫혔으나(load-bearing 실증) **좁힘의 여집합**에서 새 놓침. R-9 = 람다를 **감싸서 등록**하는 자바 관용구 9형태(`requireNonNull(L)`·`return wrap(L)`·`new Holder(L)` 등)를 브랜치가 놓침 — **main 은 잡음**.
- **🔴 핵심 교훈(리뷰어 실증·A-0055~0058 반복)**: **"음성검증(rig)은 *추가한 단언*만 지키고 *사라진 커버리지*는 못 지킨다. 판별식을 좁히는 변경은 반드시 *좁힘의 여집합*을 fresh 입력으로 친다."** — 이걸 안 해서 매번 새 놓침이 리뷰에서야 발견됨.
- **근본 한계**: 소음 vs 신호 구분 = **람다가 실제 뭘 부르는지(타입/데이터플로) 필요**. tree-sitter 는 **타입 없음** → 어림 좁힘은 항상 샘 = 구조적 무한반복.

**"고칠 수 있냐" 답**: **지금 도구(tree-sitter·타입없음)로는 깔끔히 불가.** 완전 폐쇄 = **네이티브 심볼솔버(설계 §5 정밀 parity)** 도입 시 일괄. 그때까지 **소음은 정직 고지로 감당**(과탐=안전 방향이라 남겨도 됨).

**연동 결정**:
- `java.layers.callgraph` = **`partial` 확정 유지**(supported 승격은 정밀솔버 이후로 이월 — D-076·D-097 계약).
- **TASK-038(킷) 착수 가능** — Java L3 를 **partial + O-14 소음 정직 고지**로 스냅샷(D-097 AC 그대로). 041 이월이 038 을 막지 않음(원래 순서 무관).
- 041 재개 조건 = 정밀솔버 도입 태스크에서. AC/보정 이력은 TASKS.md 에 기록 보존.

**비고**: 리뷰(타 세션)는 정상 작동했다 — 매 보정요청은 **진짜 과소탐을 잡은 정당한 반려**. 문제는 리뷰가 아니라 **태스크가 도구 한계와 싸운 것**. 이월이 AC(D-097 "소음 안 닫히면 이월·과소탐 금지 우선")가 정한 정답.

## D-104 (2026-07-22) TASK-038 리뷰 통과·머지 — 킷 Java L3 스냅샷 + 콘솔·배선 정직성 3종 (A 없음 · 통과)

**대상**: `codex/2026-07-22-task038-java-l3-kit` (`35960c0` code · `b8e519e` docs).

**결정**: **리뷰 통과 → Claude 가 `main` 머지**(비민감 — 킷 도구·콘솔·테스트·문서. D-007 구현자≠머지자).

**근거(전 AC 실측 폐쇄 — 상세 review-notes.md 2026-07-22 TASK-038 절)**:
1. **AC#1** dev↔kit md5 전량 동일(게이트 21종·정책·템플릿·tests 완전 미러) · sync 멱등 · 게이트 수 21 검산 일치. 게이트 전부 main 기리뷰본과 바이트 동일.
2. **AC#2** F1 fresh Java(픽스처 밖)가 킷 spine 으로 승인요구 · `--policies` 재바인딩 실측(F3) · 정책부재 exit 2(F5) · HAS_RANGE=0 fail-safe · CWD 기본값 하이재킹 경로 전수 감사 = 폐쇄.
3. **AC#3** `partial` 유지 + O-14·`inferred`·O-22 고지 README+manifest 양쪽 명시(D-097·D-103 준수).
4. **AC#4** selftest exit 0 — 러너 전량(리뷰 환경은 tree-sitter 가용이라 smoke 갭까지 폐쇄) · 진입점 31/31 · 뮤테이션 351.
5. **AC#5** G-1(V2 ScannerError rig 30/31 재현·exit 불변) · G-2(F4 fresh 고지·N3 load-bearing) · G-3(N4 소실/복원 · 3분류 전부 실제 level 노출 = 정직성 확대).
6. **AC#6** F2b(대상 repo 정책 3종 하이재킹 무력) · F3(override 라우팅 실소비 판별 실증) · N1(배선 제거 시 정밀탐지 소실 + 게이트 fail-closed 2차 방어 = 중복 방어 설계) · 스위트 rig 28/31 재현.

**Codex 신고 대비**: 수치 전부 일치(31/31·30/31·28/31·351) — 과장 없음. §1 보수성 통과(접촉 = kit/+handoff+summaries 뿐).

**신규 비차단**: **O-36**(실행실패 시 콘솔 이중 고지 — 과잉 방향·판정 무영향) · **O-37**(능력 JSON 파싱 실패 시 카드 not_checked 노트 미기재 — 선재 갭, **차기 킷 AC 가드 후보**) · **O-38**(`java-indirect-routing-override` 판별력 보강 권고 — F3 방향 케이스). §2B 필수질문: 셋 다 민감 변경 포착에 구멍 없음(탐지 손실·판정 변경 0) ⇒ 비차단 적법.

**후속**: 킷 버전 `0.3.4-mvp3-java-l3`. `java.layers.callgraph` `partial` 은 정밀솔버(설계 §5) 이후 재검토(D-103). O-37 은 차기 킷 태스크 AC 로 편입할 것.
