...
### TASK-041 ⏸ **이월(DEFERRED · D-103, 2026-07-22)** — Java L3 소음(O-14) 폐쇄 (Codex)  *(MVP-3 · X · Java L3-e)*

> 🛑 **착수 금지 — 이 태스크는 형 지시로 이월됐다(D-103). 브랜치 `codex/2026-07-22-task041-java-l3-noise`(+11) 는 머지하지 않는다(main 보다 나쁨 = 과소탐).** 다음 = **TASK-038(킷)**.
>
> **왜 이월하나 — 근본 원인(Codex 학습용, 명확히):**
> - **41 이 하려던 것**: Java 간접영향의 **과잉 경고(소음)** 를 줄이려고, 지연 dispatch(람다/콜백) 승격 판별식을 **좁혔다**(관련 있는 것만 승격).
> - **왜 보정→보정이 무한 반복됐나(5라운드 D-098~102·리뷰어 자기정정 10회)**: 판별식을 **좁히면(덜 잡게 하면)** 반드시 **덜 잡던 자리에서 실제 위험을 놓치는 구멍(과소탐)** 이 생긴다. 매 라운드 지난 구멍은 닫혔으나(AC 충족·load-bearing) **좁힘의 여집합(새 엣지 형태)** 에서 새 놓침이 터졌다(R-1→R-9). **핵심 교훈(리뷰어 실증)**: *"음성검증(rig)은 **추가한 단언**만 지키고 **사라진 커버리지**는 못 지킨다. 판별식을 좁히는 변경은 반드시 **좁힘의 여집합을 fresh 입력으로** 친다."*
> - **근본 한계(왜 지금 도구로는 못 닫나)**: 소음(무관 람다)과 신호(sink 로 가는 람다)를 구분하려면 **람다가 실제 무엇을 부르는지 = 타입/데이터플로 해석**이 필요하다. 그런데 tree-sitter 는 **타입을 모른다**(구조만). 타입 없는 어림 좁힘은 **항상 어딘가 샌다** → 무한 반복의 구조적 원인.
> - **안전성**: 현행 main 은 이 형태들을 전부 **과잉이라도 승인요구로 잡는다(과탐 = 안전 방향)**. 41 브랜치는 좁히다 **놓침(과소탐 = 위험 방향)** 이 생겨 **main 보다 나쁘다** → 머지 불가.
>
> **그럼 언제/어떻게 닫나**: tree-sitter 땜질이 아니라 **네이티브 심볼솔버(타입 해석) 도입 = 설계문서 §5 "정밀 parity" 옵션**이 붙을 때 일괄로. 그때까지 O-14 소음은 **정직 고지로 감당**한다(TASK-038 이 킷 README·`manifest` 에 소음 특성·`partial` 명시). `java.layers.callgraph` 는 **`partial` 확정 유지**(supported 승격은 이 정밀솔버 이후로 이월).
>
> **아래 AC/보정 이력은 기록으로 보존**(재개 시 참고). 지금은 **착수하지 말 것.**
...
**🔴 보정 지시 (D-100 · A-0056 — `9716fb5`·`5b32acc` 재리뷰 결과 · AC#11·#12·#13 은 기능 충족 + load-bearing 확인, 되돌리지 말 것)**
**새 태스크 아님 — `codex/2026-07-22-task041-java-l3-noise` 에 보정 커밋만 재제출.** 아래 2건은 리뷰어가 워크트리(`wt-fix-r6`)에 직접 패치해 **탐지축·소음축을 동시에 실측**한 뒤 확정했다(측정표는 A-0056 §4).
**⚠️ R-5 는 리뷰어(내) 처방의 결함이다** — A-0055 §3(a) 가 증거의 마지막 갈래를 "같은 caller 의 unresolved" 로 못박았고 Codex 는 그 문언을 **정확히** 구현했다(리뷰어 자기정정 8회째). **Codex 귀책 아님.** 브랜치가 아직 `main` 에 없으므로 지금 막는다.

14. **🔴 (R-5 · 차단 · 흔적 없는 과소탐) 타입 증거 판정을 *레코드-결부* 근거로 한정** — `unresolved_by_caller[caller]` 는 그 메서드 안 **모든** 미해소 호출이라, 배선 메서드에 **무관한 외부 호출 한 줄**만 있으면 증거 있음 → AC#11 승격 분기 사망 → 빈 `type_match` 로 떨어져 레코드 전량 폐기(`errors:[]`·`fail_closed:[]`·`indirect_impact:[]` **완전 무흔적**). **R-1 이 지적한 "무관한 한 줄이 판정을 뒤집는다" 가 새 자리에 그대로 재생산됐다.**
    - **🔴 신규 픽스처가 이 갈래에 구조적으로 도달하지 못한다** — `java-indirect-deferred-no-type-jdk` 는 소음 `rows.size()` 가 **`sink()`** 안에 있는데 증거 판정은 **레코드의 caller = `Flow.wire`** 만 본다 ⇒ **소음 내성을 전혀 증명하지 못하는 false-confidence 픽스처**. 삭제하지 말고 **유지하되 짝을 추가**할 것(둘의 verdict 가 같아야 한다는 것이 이 AC 의 요지).
    - **fresh 실증(`main` 2 → 브랜치 0 · 완전 무흔적)** — **B1**: 그 픽스처에서 **`rows.size()` 를 `sink()`→`wire()` 로 옮기기만** 함(의미상 동일 프로그램: 같은 호출·같은 경로·같은 sink). **B2**: 소음 = `sb.append("x")`(`StringBuilder`) ⇒ JDK 컬렉션 특정 아님. **B3**: 2단 위임(`…-two-hop` 픽스처) + 소음을 `wire()` 로. **A2**: `wire()` 한 줄 안에 `rows.size(); d.task = () -> …;`.
    - **실전 `wire()`/`init()`/`configure()` 는 로깅·널체크·컬렉션 조작이 거의 항상 섞인다** ⇒ AC#11 이 커버한다고 주장하는 관용구가 **현실 코드에선 대부분 다시 놓친다.**
    - **수정 방향(리뷰어 실측)**: **추출기가 "이 deferred 레코드를 감싼 invocation" 을 레코드 자체에 실어 주는 것이 정답**(`owner_from_invocation` 선례). 게이트가 caller 로 역추측하지 않게 한다. 근거: N4 는 `consume`(line 9)이 lambda(line 9)와 **같은 줄** = 진짜 결부 / B1 은 `rows.size()`(line 12) vs lambda(line 13) = 무관. **리뷰어 프로토타입은 그 근사로 같은 `line` 제약을 걸었다 — 임시 근사일 뿐 수용기준이 아니다.**
    - **🔴 `A2`(같은 줄 소음)까지 닫을 것** — 라인 일치 근사로는 A2 가 **잔존**한다(리뷰어 프로토타입에서도 `pass`).
    - **회귀 픽스처 3건 신설**(B1·B2·B3 형태) — 전부 `verdict: approval_required`·`exit_code: 2`. **대조군 불변 단언 필수**: `java-indirect-deferred-jdk-noise-pass`(N4) `pass` · A7 형태 `pass`. **음성검증**: 이 판정을 원복하면 신규 3건 **단독 FAIL**.
15. **🔴 (R-6 · 차단 · 흔적 없는 과소탐) 상속 폐포를 *양방향* 으로** — AC#12 는 `deferred_types` **만** 상위로 확장해 **"필드는 상위타입으로 선언하고 사용처에서 좁히는"** 방향이 그대로 뚫린다.
    - **fresh 실증(`main` 2 → 브랜치 0)** — **A4**: `interface Task extends Runnable {}` + `Runnable task; wire(){ task = () -> new Vault().transfer(); } sink(){ Task t = (Task) task; t.run(); }` (`deferred={Runnable}` 폐포 `{Runnable}` / `opaque={Task}` ⇒ 교집합 ∅). **B5**: 형제 인터페이스 캐스트.
    - **A5(하위→상위, `Task extends Mid extends Runnable`)는 잡고 A4(상위→하위)는 놓친다** — 같은 상속 한 쌍인데 **폐포를 어느 쪽에 걸었느냐로만** 갈린다. `B4`(인라인 캐스트 `((Task) task).run()`)가 잡히는 건 수신자가 미상으로 떨어지는 **다른 경로** 덕이라 반증이 아니다.
    - **수정 방향(리뷰어 실측)**: `opaque_receiver_types` 에도 동일 폐포 적용(1~3줄). 공통 조상으로 인한 과잉매칭은 **과탐 방향**이라 MVP-3 계약상 안전하며, 실측상 소음 폐쇄(N4·A7)와 스위트에 **회귀 0**이었다.
    - **회귀 픽스처 2건 신설**(A4·B5 형태) — `approval_required`. **음성검증**: 한 방향으로 되돌리면 단독 FAIL. **N4·A7 `pass` 불변 단언 필수.**
    - **🔴 리뷰어 측정표(재현 필수 · 원복 확인함)**: **(c)+(d) 적용 시** — **B1·B2·B3·A4·B5 5형태 전부 `main` 과 동일 복구(2)** · **A7(N4 형태) `pass`(0) 유지 ⇒ O-14 소음 폐쇄가 살아 있다** · **대조군 5형태 불변(2)**: A1(Q20 원형)·A3(서브인터페이스+소음)·A5(2단 상속)·B4(인라인 캐스트)·B6(`Object` 필드+익명클래스) · **스위트 212/213 유지(회귀 0)** ⇒ 소음↔탐지 트레이드오프가 아니라 **순수 개선**. **단 A2 는 잔존 — 반드시 추가로 닫을 것.**
16. **무회귀(보정분)**: 스위트 전량 · `mutation-check.sh` · **`PYTHONHASHSEED` 0/1/999/12345 md5 동일** · Python 골든패스 불변(`default` 가 `main` 과 동일) · **exit `0`/`2` 뿐** · **`kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md`·Claude 소유 무접촉**. 재제출 전 **`origin/main` 머지**(collab-protocol §5.1 — 미동기화 **11회째**).

**🔴 보정 지시 (D-101 · A-0057 — `a59d819`·`ecd906a` 재리뷰 결과 · AC#14·#15·#16 은 기능 충족 + load-bearing 확인, 되돌리지 말 것)**
**새 태스크 아님 — `codex/2026-07-22-task041-java-l3-noise` 에 보정 커밋만 재제출.** 아래 2건은 리뷰어가 워크트리(`wt-fix-r7`)에 직접 패치해 **탐지축·소음축을 동시에 실측**한 뒤 확정했다(측정표는 A-0057 §4).
**⚠️ R-7 은 리뷰어(내) 처방 문언의 과소지정이다** — A-0056 AC#14 는 *"이 레코드를 **감싼** invocation"* 이라고만 썼고 Codex 는 **"가장 가까운 조상 `method_invocation`"** 으로 문언대로 정확히 구현했다. 내가 뜻한 것은 *"람다가 **인자로 넘어간** 그 호출"* 이었다(리뷰어 자기정정 9회째). **Codex 귀책 아님.** 브랜치가 아직 `main` 에 없으므로 지금 막는다.
**✅ AC#14 가 요구한 `A2`(같은 줄 소음)는 실제로 닫혔다** — 신규 픽스처 3건이 소음을 람다와 **같은 줄**에 두었고 fresh A2·B1·C10 이 복구됐다. **A-0056 이 지적한 false-confidence 픽스처 문제도 해소.**

17. **🔴 (R-7 · 차단 · 과소탐 · 직전 커밋 대비 순회귀 포함) 타입 증거를 *"람다가 인자로 넘어간 호출"* 로 한정** — 현행 조상 순회(`extract-java-callgraph.py` 람다 분기)는 **"람다가 그 호출의 인자인가"** 를 묻지 않고 **대입식에서 끊지도 않아**, `d.task = () -> …` 라는 **실제 등록 행위를 감싼 무관한 호출**이 그대로 증거가 된다. `precise_receiver_type(object_node=None) → simple_name(current_owner)`(자기 클래스명)가 이를 증폭한다.
    - **fresh 실증(전부 픽스처 밖 · 변경 = `Vault.transfer` 본문 1줄)** — **C2**: `void keep(Object o) {}` + `void wire() { keep(d.task = () -> new Vault().transfer()); }` ⇒ `main` **2** · **직전 `5b32acc` 2** · 브랜치 **0**. **🔴 직전 커밋 대비 순회귀다** — `keep(...)` 은 리포 안에 선언된 **해소되는** 호출이라 `unresolved` 에 등장하지 않고, 그래서 옛 caller 스캔은 증거 0 → 승격 → **탐지**했다. **새 코드는 없던 증거를 만들어낸다.** **C1**: `Objects.requireNonNull(d.task = () -> …)` ⇒ 증거 `Objects`. **C6**: `rows.add(String.valueOf(d.task = () -> …))` ⇒ 증거 `String`(O-30 타입인자 누출이 다시 판정을 좌우).
    - **🔴 게이트는 C2 와 N4 를 구분할 수단이 없다** — N4(`consume(() -> log("head"))`)를 `pass` 로 지키는 값도 **똑같은 `Flow`** 다. 유일한 실질 차이는 **람다가 그 호출의 *인자*인가(N4) / 그 호출이 *대입식을 감쌌을 뿐*인가(C2)**. 이 구분을 코드에 넣지 않으면 하나를 살리는 순간 다른 하나가 죽는다.
    - **수정 방향(리뷰어 실측 · 12줄)**: 조상 순회에 두 제약. ① **`assignment_expression`·`variable_declarator` 를 만나면 즉시 중단** — 그 지점이 람다를 실제로 **바인딩**하는 곳이고 그 바깥 호출은 이 람다와 무관하다. ② **`method_invocation` 에 도달했을 때, 올라온 자식이 그 호출의 `arguments` 필드일 때만** `precise_receiver_type` 을 채택.
    - **회귀 픽스처 3건 신설**(C2·C1·C6 형태) — 전부 `verdict: approval_required`·`exit_code: 2`. **대조군 불변 단언 필수**: `java-indirect-deferred-jdk-noise-pass`(N4) `pass` · A7 형태 `pass` · 기존 신규 5픽스처 `approval_required` 유지. **음성검증**: 두 제약을 하나씩 원복하면 신규 케이스가 **단독 FAIL**.
18. **🟠 (R-8 · 차단 · 1토큰 · 보수 방향 역전) dedup 동점규칙을 보수 방향으로** — `rank = (bool(receiver), not bool(enclosing), receiver, enclosing)` 는 최소값이 이기므로 **`enclosing` 있는 쪽을 선호**한다. `enclosing` 은 **탐지를 끄는 값**이라 방향이 거꾸로다.
    - lambda 레코드 dedup key 가 `(caller, "lambda_dispatch", "lambda", line, dispatch_targets)` 라 **같은 줄의 서로 다른 두 람다가 한 레코드로 합쳐진다.** fresh **C3**: `void wire() { Objects.requireNonNull(() -> hashCode()); d.task = () -> new Vault().transfer(); }` ⇒ **소음 람다(enclosing=`Objects`)가 실제 등록 람다(enclosing 없음)를 밀어내** 증거 있음 → 폐기 → `pass`.
    - **정직하게 적는다**: C3 는 **직전 커밋 대비 순회귀가 아니다**(5b32acc 도 `pass`). 순회귀를 만드는 변형(C7 = in-repo 래퍼, C11 = 오버로드)도 **직접 만들어 돌려봤으나 전부 `approval_required` 로 생존**(`bool(receiver)` 첫 성분이 먼저 걸러서다). **그럼에도 차단** — 방향이 명백히 보수성 반대이고, 수정이 **`not` 두 개 제거뿐**이며, AC#17 적용 후 `enclosing` 의 영향력이 커지므로 그대로 두면 위험만 남는다.
    - **수정 방향**: `rank`·`previous_rank` 양쪽의 `not bool(enclosing)` → `bool(enclosing)`. **회귀 픽스처 1건 신설**(C3 형태) — `approval_required`. **음성검증**: 되돌리면 단독 FAIL. **N4·A7 `pass` 불변 단언 필수.**
    - **🔴 리뷰어 측정표(재현 필수 · 원복 확인함)**: **(e)+(f) 적용 시** — **C1·C2·C3·C6 4형태 복구(2)** · **A2·B1·A4·B5·C10 유지(2)** · **A7(N4 형태) `pass`(0) 유지 ⇒ O-14 소음 폐쇄가 살아 있다** · **대조군 9형태 불변(2)**: A1(Q20 원형)·A5(2단 상속)·B4(인라인 캐스트)·B6(`Object` 필드+익명클래스)·C5(체인 수신자)·C7·C11·C4b(in-repo 고차함수)·C4c(익명클래스 안 등록) · **스위트 217/218 유지(회귀 0)** ⇒ 소음↔탐지 트레이드오프가 아니라 **순수 개선**.
19. **무회귀(보정분)**: 스위트 전량 · `mutation-check.sh` · **`PYTHONHASHSEED` 0/1/999/12345 md5 동일** · Python 골든패스 불변(`default` 가 `main` 과 동일) · **exit `0`/`2` 뿐** · **`kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md`·Claude 소유 무접촉** · 재제출 전 **`origin/main` 재머지**(직전 재제출에서 11회 만에 이행됨 — 유지할 것).

**✅ AC#18 폐쇄 확인(D-102)** — RIG-C(원복) 220/222 `…same-line-lambda-dedup` 단독 FAIL · 픽스처의 `(Runnable)` 캐스트 의존 의심 → **캐스트 없는 fresh C3** 로 독립검증(`wt-br` 2 ↔ `rigC` 0) ⇒ 픽스처 정직.
**✅ AC#17① 폐쇄 확인(D-102)** — RIG-A(`break` 2줄 제거) 218/222 신규 3건 단독 FAIL · fresh C2·C1·C6 로 **C2 순회귀 복구** 확인.
**⚠️ AC#17② 는 무효(inert) — 리뷰어 처방의 결함, 요구 철회(D-102).** `child == arguments` 가드는 **RIG-B**(제거) FAIL **0건** · **RIG-D**(계측: `child != arguments` 인데 `precise_receiver_type` 이 non-None 인 순간 기록) **스위트 전량 0회**. 원인: ① 이 대입/선언자에서 끊은 뒤 람다가 `arguments` 아닌 경로로 invocation 에 닿는 형태는 **`object` 필드(캐스트·괄호 경유)** 뿐인데 `precise_receiver_type()` 이 그 노드타입을 어느 갈래에도 태우지 않아 **항상 `None`**. **가드는 무해한 방어코드로 유지하되 회귀 픽스처 요구는 철회한다.**

20. **🔴 (R-9 · 차단 · 과소탐 · `main` 대비 탐지 손실) 결부 경계를 *양방향*으로** — AC#17① 은 **람다→조상** 방향만 끊는다. **채택한 `method_invocation` *위*에 대입·선언·`return` 이 있어도 그 자리에서 무조건 `break` 하므로 아무 일도 일어나지 않는다.** ⇒ 람다가 진짜로 그 호출의 인자이면서 **그 호출의 결과가 레지스트리에 바인딩되는** 형태 — 즉 **래핑 등록 관용구** — 가 통째로 뚫린다. 증거는 또다시 `precise_receiver_type(object=None) → simple_name(owner)` = **자기 클래스명**(`Flow`)이거나 무관한 JDK 타입(`Objects`)이다.
    - **fresh 실증 9형태(전부 픽스처 밖 · 변경 = `Vault.transfer` 본문 1줄) — `main` 2 → 브랜치 `pass`(0)**: **H2** `d.task = wrap(() -> …)` · **H2c** `d.task = Objects.requireNonNull((Runnable) () -> …)` · **H2d** `Runnable r = wrap(() -> …); d.task = r;` · **H2e** `d.task = svc.wrap(() -> …)` · **H3** `Runnable make(){ return wrap(() -> …); } … d.task = make();` · **H7** 2중 래핑 · **H9** 캐스트 경유 · **K2** 배열 초기화 경유 · **K3** `svc.register(new Holder(() -> …))`.
    - **K3 는 성격이 다르다** — `object_creation_expression` 이 경계가 아니라 순회가 **생성자를 뚫고 지나가** 바깥 `svc.register` 의 수신자를 증거로 삼는다. **R-7 이 지적한 오귀속이 형태만 바꿔 남은 것.**
    - **정직하게 적는다**: 커밋별 이분탐색 결과 H2 계열은 **브랜치 첫 코드 커밋 `1f0c60b` 부터 이미 `pass`** 다 ⇒ **이 델타의 순회귀는 아니다.** 그러나 **`main` 은 9형태를 전부 잡으므로 머지하면 `main` 이 지금 잡는 탐지를 잃는다** ⇒ 차단.
    - **대조군은 전부 생존**(A1·A2·C10·C1·C2·C3·C6·H1c·H4 메서드참조·H5 삼항·H6 연쇄대입·H10 static 초기화자·K1 필드 초기화자 = `approval_required`) · **소음 폐쇄도 생존**(N4·N5·N6 = `pass`) ⇒ 구멍은 **"래퍼 호출을 거쳐 바인딩/반환되는 등록"** 한 갈래. 실전 Java 의 가장 흔한 등록 관용구(`disposable = obs.subscribe(…)` · `handle = bus.register(…)` · `return Wrappers.decorate(() -> …)`)가 정확히 여기 든다.
    - **수정 방향(리뷰어 실측 · 현행 대비 4줄)**: `method_invocation` 채택 후 **`break` 대신 `adopted` 플래그로 순회를 계속**(채택은 첫 invocation 1회로 한정 — R-7 재발 금지) · 경계집합에 **`return_statement`·`object_creation_expression` 추가** · 경계를 만나면 **채택한 증거까지 `None` 으로 폐기**.
    - **회귀 픽스처 4건 신설** — ① H2(무자격 미해소 래퍼) ② H3(반환 경유) ③ K2 또는 H7(배열 초기화 / 2중 래핑) ④ K3(생성자 인자 오귀속). 전부 `verdict: approval_required`·`exit_code: 2`. **대조군 불변 단언 필수**: N4 `pass` · A7 `pass` · AC#14/#15/#17/#18 신규 픽스처 9건 `approval_required`. **음성검증**: 「위쪽 경계 검사」와 「`object_creation_expression` 경계」를 **하나씩 원복**하면 신규 케이스가 **단독 FAIL**.
    - **🔴 리뷰어 측정표(재현 필수 · 단계별 원복 확인함)**: `wt-fix3` 적용 시 — **9형태 복구(2)** · **대조군 13형태 불변(2)** · **N4·N5·N6 `pass`(0) 유지 ⇒ O-14 소음 폐쇄 생존** · **스위트 221/222 유지(회귀 0)** ⇒ 순수 개선. 중간 단계도 남긴다: `adopted` 만 도입 = 6형태 / `+return_statement` = 8형태 / `+object_creation_expression` = 9형태, **셋 다 221/222**.
21. **무회귀(보정분 · AC#20)**: 스위트 전량 · `mutation-check.sh` · **`PYTHONHASHSEED` 0/1/999/12345 md5 동일** · Python 골든패스 불변(`default` 가 `main` 과 동일) · **exit `0`/`2` 뿐** · **`kit/`·`policies/`·`docs/`·`templates/`·`AGENTS.md`·Claude 소유 무접촉** · 재제출 전 **`origin/main` 재머지**.

**비차단(차기 · 감점만 · D-102 갱신)**: **🟡 O-34 신규** — `precise_receiver_type(object_node=None) → simple_name(current_owner)`. 무자격 호출(`wrap(...)`)의 "수신자 타입" 을 **자기 클래스명**으로 단정하는데 이는 *람다의 함수형 타입*과 아무 관계가 없다. **R-7 과 R-9 가 둘 다 이 값에서 증폭됐다.** AC#20 은 **경계**로 우회할 뿐 **값 자체는 그대로 남는다** ⇒ 무자격 호출은 증거를 **`None`(증거 없음 = 보수)** 으로 두거나 최소한 자기 클래스명을 `enclosing_invocation_receiver_type` 후보에서 제외. **🟡 O-35 신규(방법론)** — **AC 가 요구한 음성검증이 원리적으로 불가능한 경우가 있다**(AC#17②). 앞으로 rig 요구를 쓸 때는 **"해당 갈래에 도달하면서 값이 non-None 인 입력이 존재하는가" 를 처방 단계에서 먼저 확인**하고, 이번에 쓴 **계측 리그(RIG-D: 가드가 막은 순간을 stderr 로 기록) 패턴을 상설화**한다. **🔴 O-33 격상(승격 차단조건 추가)** — R-9 9형태가 **또 전부 조용히** 통과했다(`verdict: pass`·`exit 0`·`errors:[]`·`fail_closed:[]`). **R-1·R-4·R-5·R-7·R-9 다섯 계열이 모두 같은 무신호 경로로 샜다** ⇒ 억제 사유(`type_mismatch`/`evidence_present`)를 카운터나 `notes` 로 1급 표기하는 것을 **`supported` 승격 차단조건에 추가**한다.

**비차단(D-101 이월)**: **🟡 O-32 신규(승격 차단조건)** — `nested_in_deferred_body()` 가 **지연 본문 안에 중첩된 람다를 순회에서 통째로 제외**한다. fresh **C4**(`Optional.ifPresent(c -> { d.task = () -> new Vault().transfer(); })`): `main` 2 → 직전 0 → 브랜치 0 → 리뷰어 프로토타입 0. **이 델타의 회귀가 아니고**(직전 커밋도 동일), **리포 내 고차함수(C4b)·익명클래스(C4c) 형태는 4개 트리 전부에서 탐지**되므로 범위는 *"JDK/미해소 고차 호출의 콜백 안에서 등록"* 으로 좁다. 닫으려면 N4 소음 폐쇄(그 `consume` 자체가 미해소 호출이다)와 정면충돌하므로 **패치가 아니라 설계 판단**이 필요 ⇒ 보정 AC 로 강제하지 않고 **`supported` 승격 차단조건**으로 못박는다. **🟡 O-33 신규** — 폐기된 deferred 레코드에 **거버넌스 신호가 없다.** `coverage.unevaluated` 에는 남지만 그건 모든 실행에 항상 있는 추출기 원본 패스스루라 "sink 폐쇄에 닿는 등록이 억제됐다" 를 뜻하지 않는다(A-0056 의 *"완전 무흔적"* 표현은 부정확 — 정확히는 **신호 부재**). **R-1·R-4·R-5·R-7 이 매번 조용히 통과한 근본 원인.** 폐쇄 방향: 억제 사유(`type_mismatch` / `evidence_present`)를 카운터나 `notes` 로 1급 표기 ⇒ 다음 과소탐이 리뷰가 아니라 **출력**에서 드러난다. **🟡 O-31 부분해소** — 소음 폐쇄의 줄-배치 민감성은 AC#14 로 한 방향이 사라졌다(같은 줄 소음이 이제 탐지). 남은 민감성은 dedup key 의 `line` 에서 오며 AC#18 이 그 영향을 보수화한다. **O-29** `precise_receiver_type()` 의 `this.` 갈래가 `split(".",2)[1]` 로 두 번째 토큰만 봐서 `this.holder.task.run()` 을 **`Holder`** 로 확신(접두 없는 `holder.task.run()` 은 `None` 으로 정확 — **`this.` 하나로 갈린다**). **AC#15 양방향 폐포가 들어오면 틀린 타입이 더 넓게 퍼지므로** 같이 닫는 편이 안전 — `this.` 뒤 토큰이 하나뿐일 때만 조회, 체인이면 `None`. **🔴 O-30 격상 정정** `List<String> rows` 바인딩의 **타입 인자 누출**(`receiver_type: String`)은 A-0055 가 "판정 무영향" 으로 적었으나 **이제 아니다** — B1 에서 **그 잘못된 `String` 이 바로 "타입 증거 있음" 을 켜서 탐지를 끄는 값**이다. AC#14 로 증거를 레코드-결부로 좁히면 부수 완화되나 **바인딩 단계 자체는 별건으로 남는다.** **O-31 신규** 소음 폐쇄가 **줄 배치에 민감**(N4 형태를 한 줄로 두면 `pass`, 소음을 별도 줄로 내리면 `approval_required`) — 과대 방향이라 비차단이나 증적 일관성 문제.
