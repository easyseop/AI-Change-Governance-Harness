...
4. **정직 한계 명시**: 위험 프리미티브를 안 쓰는 "의미상 민감 함수"(순수 계산 정산 로직 등)는 못 뽑음 → 출력 헤더/문서에 이 한계를 표기(과대선전 방지).
...
**근거**: 리뷰에서 blocked→pass 변조 시 정상 FAIL(39/40) 확인 — 이 수동 절차를 상설화.

### TASK-018 ☐ 정책 완화 방향 별도 승인 게이트 `check-policy-change.py`  (Codex)  *(개선점 #4/#5)*
**목적**: 통제 장치의 **자기 무력화**를 막는다. 정책 파일(`policies/*.yaml`) 변경 중 **강화(등급↑·규칙 추가)** 는 통과, **완화(등급↓·규칙/항목 삭제·범위 축소)** 는 **사람 승인 강제**.
**수용기준**:
